...
def api_list(token):
    ...
    files = []
    try:
        HIDDEN_ITEMS = {"venv", "__pycache__", ".git", "node_modules", "output.log", ".env.bak"}
        for entry in sorted(os.scandir(target), key=lambda e: (not e.is_dir(), e.name.lower())):
            if entry.name in HIDDEN_ITEMS:
                continue
...
def index():
    return """
<!DOCTYPE html>
<html><head><meta charset="UTF-8"><title>God Madara Hosting</title>
<style>
body{background:#0d1117;color:#c9d1d9;font-family:'Segoe UI',sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;gap:15px;}
h1{color:#f0c040;font-size:2.5em;}p{color:#8b949e;font-size:1.1em;}
.badge{background:#238636;color:white;padding:8px 20px;border-radius:20px;font-size:0.9em;margin-top:10px;}
</style></head>
<body>
<h1>🌟 God Madara Hosting Bot</h1>
<p>Bot is running! Use Telegram to manage your projects.</p>
<span class="badge">✅ Online</span>
<p style="font-size:0.8em;margin-top:20px;">File Manager links are generated from the Telegram bot.</p>
</body></html>
"""
