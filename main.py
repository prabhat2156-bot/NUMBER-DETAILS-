...
# Semaphore: max 15 concurrent pip/npm installs (safe for 12 GB RAM server)
...
async def _startup_restart_one(proj: dict, bot_locked: bool) -> None:
    """Helper: install deps + start one project on startup. Runs in parallel via gather."""
    uid  = proj["user_id"]
    name = proj["name"]
    try:
...
async def auto_restart_on_startup():
    """Restart ALL projects in parallel on bot startup. Respects bot_lock & maintenance."""
    await asyncio.sleep(15)  # brief wait for bot to finish initialising
    try:
        ...
        logger.info(f"Auto-restart on startup: launching {len(running_projects)} projects (batched)...")
        bot_locked = await is_bot_locked()

        # Launch in batches of 3 to avoid RAM spike on Render free plan
        BATCH = 3
        for i in range(0, len(running_projects), BATCH):
            batch = running_projects[i:i + BATCH]
            await asyncio.gather(*[_startup_restart_one(proj, bot_locked) for proj in batch], return_exceptions=True)
            gc.collect()
...
async def setup_venvs_background():
    """Create missing venvs for projects at startup — batched 3 at a time to limit RAM on Render free plan."""
    await asyncio.sleep(5)  # let bot finish initialising first
    try:
        all_projects = await projects_col.find(
            {}, {"user_id": 1, "name": 1}  # only fetch fields we need
        ).to_list(length=500)
        logger.info(f"setup_venvs_background: checking {len(all_projects)} projects (batched)...")
        BATCH = 3  # Render free plan: max 3 venv installs running at once
        for i in range(0, len(all_projects), BATCH):
            batch = all_projects[i:i + BATCH]
            await asyncio.gather(*[_setup_venv_one(proj) for proj in batch], return_exceptions=True)
            gc.collect()
