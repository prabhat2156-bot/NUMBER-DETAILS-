...
async def cb_reinstall_reqs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sem = INSTALL_SEMAPHORE or asyncio.Semaphore(15)
    async with sem:
...
async def keep_alive_task():
    """Ping health endpoint every 5 minutes to prevent Render sleep + clean expired tokens."""
...
async def auto_restart_on_startup():
    ...
    try:
        ...
        logger.info(f"Auto-restart on startup: launching {len(running_projects)} projects in parallel...")
        bot_locked = await is_bot_locked()

        # Launch all projects simultaneously — no sequential waiting
        await asyncio.gather(
            *[_startup_restart_one(proj, bot_locked) for proj in running_projects],
...
async def setup_venvs_background():
    """Create missing venvs for ALL projects in parallel at startup."""
    await asyncio.sleep(5)  # let bot finish initialising first
    try:
        all_projects = await projects_col.find({}).to_list(length=10000)
        logger.info(f"setup_venvs_background: checking {len(all_projects)} projects in parallel...")
        await asyncio.gather(
            *[_setup_venv_one(proj) for proj in all_projects],
            return_exceptions=True,
        )
...
async def post_init(app: Application):
    ...
    # 15 concurrent installs — safe for 12 GB RAM Render server
    INSTALL_SEMAPHORE = asyncio.Semaphore(15)
