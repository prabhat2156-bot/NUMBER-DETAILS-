...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_banned(user.id):
        await update.message.reply_text("🚫 You are banned\\. Contact owner\\.", parse_mode=ParseMode.MARKDOWN_V2)
        return
...
async def cb_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await is_banned(user.id):
        await safe_edit(query, "🚫 You are banned\\. Contact owner\\.", parse_mode=ParseMode.MARKDOWN_V2)
        return
    ...
    await safe_edit(query, text, reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.MARKDOWN_V2)
...
async def cb_bot_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if meta:
        ...
        backup_size = escape_md(fmt_bytes(meta.get("total_size", 0)))
    ...
    await safe_edit(query, text, reply_markup=kb, parse_mode=ParseMode.MARKDOWN_V2)
...
async def cb_my_projects(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not projects:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="back_start")]])
        await safe_edit(query, "📂 *My Projects*\n\nYou have no projects yet\\.", reply_markup=kb, parse_mode=ParseMode.MARKDOWN_V2)
        return
    ...
    await safe_edit(query, "📂 *My Projects*\n\nSelect a project:", reply_markup=InlineKeyboardMarkup(kb_rows), parse_mode=ParseMode.MARKDOWN_V2)
...
def project_dashboard_text(p: dict) -> str:
    ...
    if status == "running" and p.get("started_at"):
        ...
        uptime  = fmt_uptime(elapsed)
    last_run = p["last_run"].strftime("%Y-%m-%d %H:%M UTC") if p.get("last_run") else "Never"
    exit_code = str(p.get("exit_code")) if p.get("exit_code") is not None else "None"
    run_cmd   = p.get("run_command") or "Not set"
    created   = p["created_date"].strftime("%Y-%m-%d") if p.get("created_date") else "N/A"
    ...
    return (
        ...
        f"🔹 Run Command: `{escape_md(run_cmd)}`\n"
        f"📅 Created: `{escape_md(created)}`"
    )
...
def project_dashboard_kb(user_id: int, project_name: str) -> InlineKeyboardMarkup:
    pn = project_name
...
async def cb_project_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not p:
        await safe_edit(query, "❌ Project not found\\.", parse_mode=ParseMode.MARKDOWN_V2)
        return

    await safe_edit(query, project_dashboard_text(p), reply_markup=project_dashboard_kb(uid, name), parse_mode=ParseMode.MARKDOWN_V2)
...
async def start_project_process(uid: int, name: str) -> dict:
    ...
    # Replace 'python'/'python3' at position 0 with venv python
    ...
    return await get_project(uid, name)
...
async def cb_run(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if p.get("status") == "running" and p.get("pid"):
        if psutil.pid_exists(p["pid"]):
            await safe_edit(query, "▶️ Project is already running\\.", parse_mode=ParseMode.MARKDOWN_V2)
            return
    ...
    await safe_edit(query, f"▶️ Starting *{escape_md(name)}*\\.\\.\\.\\.", parse_mode=ParseMode.MARKDOWN_V2)
    ...
    try:
        ...
        await safe_edit(
            ...
            reply_markup=project_dashboard_kb(uid, name),
            parse_mode=ParseMode.MARKDOWN_V2,
        )
    except Exception as e:
        await safe_edit(query, f"❌ Failed to start: {escape_md(str(e))}", parse_mode=ParseMode.MARKDOWN_V2)
...
async def cb_restart(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if p.get("admin_stopped"):
        await safe_edit(query, "⚠️ Your project was stopped by admin\\. Contact owner\\.", parse_mode=ParseMode.MARKDOWN_V2)
        return

    await safe_edit(query, f"🔄 Restarting *{escape_md(name)}*\\.\\.\\.\\.", parse_mode=ParseMode.MARKDOWN_V2)
    ...
    try:
        ...
        await safe_edit(query, f"❌ Restart failed: {escape_md(str(e))}", parse_mode=ParseMode.MARKDOWN_V2)
...
async def editcmd_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    p   = await get_project(uid, name)
    kb  = project_dashboard_kb(uid, name)
...
async def cb_delete_yes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, f"✅ Project *{escape_md(name)}* deleted\\.", reply_markup=kb, parse_mode=ParseMode.MARKDOWN_V2)
...
async def new_project_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc:
        await update.message.reply_text("Please send a file document.", parse_mode=ParseMode.MARKDOWN_V2)
        return NEW_PROJECT_FILES
...
async def _finalize_new_project(update: Update, context: ContextTypes.DEFAULT_TYPE, via_message: bool):
    ...
    send = update.message.reply_text if via_message else (
        lambda t, **kw: update.callback_query.edit_message_text(t, **kw)
    )
...
async def new_project_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text("❌ Cancelled\\.", reply_markup=kb, parse_mode=ParseMode.MARKDOWN_V2)
    return ConversationHandler.END
...
async def cb_admin_backup_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, result_text, reply_markup=kb, parse_mode=ParseMode.MARKDOWN_V2)
...
async def cb_admin_user_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, "\n".join(lines), reply_markup=InlineKeyboardMarkup(kb_rows), parse_mode=ParseMode.MARKDOWN_V2)
...
async def cb_admin_running(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for p in running:
        ...
        fname  = escape_md(user_doc.get("first_name", "Unknown")) if user_doc else "Unknown"
        uname  = escape_md(f"@{user_doc['username']}" if user_doc and user_doc.get("username") else "no-username")
        pid    = p.get("pid", "N/A")
        uptime = "N/A"
        if p.get("started_at"):
            elapsed = (datetime.now(timezone.utc) - p["started_at"]).total_seconds()
        ...
        pdir   = project_dir(p["user_id"], p["name"])
        files  = ", ".join(os.listdir(pdir)[:5]) if os.path.exists(pdir) else "N/A"
        ...
        lines.append(
            ...
            f"User ID: {uname}\n"
            f"Project PID: `{pid}`\n"
            f"Running Time: `{escape_md(uptime)}`\n"
            f"Script: `{escape_md(files)}`"
        )
        kb_rows.append([
            InlineKeyboardButton("⏹ Stop",    callback_data=f"admin_stop:{p['user_id']}:{p['name']}"),
        ])
...
async def cb_admin_give_premium(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, "💎 *Give Premium*\n\nSend the user ID:", parse_mode=ParseMode.MARKDOWN_V2)
...
async def admin_give_premium_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        await update.message.reply_text("❌ Invalid ID\\. Send a numeric user ID:", parse_mode=ParseMode.MARKDOWN_V2)
        return ADMIN_GIVE_PREMIUM_ID
    ...
    try:
        await update.get_bot().send_message(uid, "🎉 You have been granted *Premium*\\! Enjoy unlimited projects\\!", parse_mode=ParseMode.MARKDOWN_V2)
    except Exception:
        pass
    await update.message.reply_text(f"✅ Premium granted to `{uid}`\\.", parse_mode=ParseMode.MARKDOWN_V2)
    return ConversationHandler.END
...
async def admin_remove_premium_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ Premium removed from `{uid}`\\.", parse_mode=ParseMode.MARKDOWN_V2)
    return ConversationHandler.END
...
async def admin_temp_premium_dur(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not m:
        await update.message.reply_text("❌ Invalid format\\. Use `24h` or `7d`:", parse_mode=ParseMode.MARKDOWN_V2)
        return ADMIN_TEMP_PREMIUM_DUR
    ...
    try:
        await update.get_bot().send_message(uid, f"🎉 You received *Temp Premium* for {escape_md(text)}\\!", parse_mode=ParseMode.MARKDOWN_V2)
    except Exception:
        pass
    await update.message.reply_text(f"✅ Temp premium set for `{uid}` — expires {escape_md(expiry.strftime('%Y-%m-%d %H:%M UTC'))}\\.", parse_mode=ParseMode.MARKDOWN_V2)
    return ConversationHandler.END
...
async def admin_ban_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ User `{uid}` banned and all projects stopped\\.", parse_mode=ParseMode.MARKDOWN_V2)
    return ConversationHandler.END
...
async def admin_unban_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await update.get_bot().send_message(uid, "✅ You have been unbanned\\! You can use the bot again\\.", parse_mode=ParseMode.MARKDOWN_V2)
    except Exception:
        pass
    await update.message.reply_text(f"✅ User `{uid}` unbanned\\.", parse_mode=ParseMode.MARKDOWN_V2)
    return ConversationHandler.END
...
async def cb_admin_broadcast_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, "📢 *Broadcast Menu*", reply_markup=kb, parse_mode=ParseMode.MARKDOWN_V2)
...
async def cb_admin_broadcast_all(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, "📢 *Broadcast All*\n\nSend the message:", parse_mode=ParseMode.MARKDOWN_V2)
...
async def cb_admin_send_to_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, "📩 *Send to User*\n\nSend the target user ID:", parse_mode=ParseMode.MARKDOWN_V2)
...
async def admin_send_user_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Send the message:", parse_mode=ParseMode.MARKDOWN_V2)
...
async def admin_send_user_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Sent to `{uid}`\\.", parse_mode=ParseMode.MARKDOWN_V2)
    except Exception as e:
        await update.message.reply_text(f"❌ Failed: {escape_md(str(e))}", parse_mode=ParseMode.MARKDOWN_V2)
    return ConversationHandler.END
...
async def admin_conv_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.callback_query:
        await update.callback_query.answer()
    await (update.effective_message).reply_text("❌ Cancelled\\.", parse_mode=ParseMode.MARKDOWN_V2)
    return ConversationHandler.END
...
async def restore_from_backup():
    ...
    async for backup in backups:
        ...
        # Recreate virtualenv for this project (async, non-blocking)
        ...
        if not os.path.exists(venv_dir):
            try:
                ...
                # Reinstall requirements if exists
    ...
    logger.info(f"Restore complete: {restored_projects} projects, {restored_files} files")
...
def build_application() -> Application:
    ...
    new_proj_conv = ConversationHandler(
        ...
        states={
            NEW_PROJECT_NAME:  [MessageHandler(filters.TEXT & ~filters.COMMAND, new_project_name)],
            NEW_PROJECT_FILES: [
    ...
    editcmd_conv = ConversationHandler(
        ...
        states={
            EDIT_RUN_CMD: [MessageHandler(filters.TEXT & ~filters.COMMAND, editcmd_receive)],
        },
    ...
    app.add_handler(CallbackQueryHandler(cb_delete_confirm,    pattern=r"^delete:[^y]"))
