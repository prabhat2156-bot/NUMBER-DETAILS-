...
async def cb_reinstall_reqs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if os.path.exists(pkg_json) and not os.path.exists(req_path):
        await safe_edit(
            query,
            f"📦 *Installing npm packages for {escape_md(name)}...*\n\nPlease wait.",
            parse_mode=ParseMode.MARKDOWN,
        )
        try:
        ...
        await safe_edit(query, msg, reply_markup=kb, parse_mode=ParseMode.MARKDOWN)
        return
    ...
    async def update_status(step: str):
        ...
        try:
            await query.edit_message_text(
                f"📦 *Reinstalling requirements for {name}...*\n\n{body}",
                parse_mode=ParseMode.MARKDOWN,
            )
        except Exception:
            pass
    ...
    if not os.path.exists(pip_path):
        await update_status("📁 Creating virtual environment...")
        try:
    ...
    await update_status("⬆️ Upgrading pip...")
    try:
        proc = await asyncio.wait_for(
    ...
    await update_status("📦 Installing requirements (this may take a while)...")
    try:
        proc = await asyncio.wait_for(
            create_subprocess_exec(
...
async def _finalize_new_project(update: Update, context: ContextTypes.DEFAULT_TYPE, via_message: bool):
    ...
    if os.path.exists(pkg_json_path):
        await update_status("📦 Installing npm packages...", results)
        try:
            proc_n = await asyncio.wait_for(
                create_subprocess_exec(
                    "npm", "install", "--no-audit", "--no-fund",
            ...
            if proc_n.returncode == 0:
                ...
                results.append(f"❌ npm install failed: {stderr_n.decode()[:300]}")
        except asyncio.TimeoutError:
