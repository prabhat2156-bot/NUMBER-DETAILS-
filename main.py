...
mongo_client = AsyncIOMotorClient(MONGODB_URI, maxPoolSize=5, minPoolSize=1)
...
INSTALL_SEMAPHORE   = None  # created inside event loop in post_init
LOCAL_LOG_MAX_BYTES = 1 * 1024 * 1024  # 1 MB — rotate logs larger than this
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != OWNER_ID and await is_maintenance_mode():
...
async def cb_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user.id != OWNER_ID and await is_maintenance_mode():
        await safe_edit(query, "🔧 *Bot is under maintenance.* Please check back later.", parse_mode=ParseMode.MARKDOWN)
        return
...
async def cb_project_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid   = query.from_user.id
    raw   = query.data
    force = raw.startswith("reinstall_reqs_force:")
    name  = raw.split(":", 1)[1]
...
async def cb_reinstall_reqs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if p.get("status") == "running" and not force:
        await safe_edit(
            query,
            f"⚠️ *{escape_md(name)} is currently running!*\n\n"
            "Installing requirements while a project runs can cause OOM crash on 512 MB RAM.\n\n"
            "Stop the project first (recommended), or continue anyway.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Continue Anyway", callback_data=f"reinstall_reqs_force:{name}")],
                [InlineKeyboardButton("⏹ Stop First",      callback_data=f"stop:{name}")],
...
async def start_project_process(uid: int, name: str) -> dict:
    ...
    Refuses if RAM is critically low to avoid OOM on 512 MB Render.
    """
    # ── Layer 1: RAM gate — refuse if too little free memory ──────────
    ram = psutil.virtual_memory()
    ram_used_mb = ram.used / (1024 * 1024)
    if ram_used_mb > RAM_SAFE_LIMIT_MB:
        logger.warning(
            f"RAM gate triggered for {uid}:{name} — "
            f"{ram_used_mb:.0f} MB used > {RAM_SAFE_LIMIT_MB} MB limit"
        )
        raise RuntimeError(
            f"\u26a0\ufe0f RAM too high ({ram_used_mb:.0f} MB used). "
            f"Stop another project first, then retry."
        )
    ...
    rotate_log_if_needed(log_path)  # rotate before starting — fresh log
    ...
    # ── Layer 2: Memory-saving env vars for child process ──────────────
    import copy
    proc_env = copy.copy(os.environ)
    # Return freed memory to OS faster (glibc malloc trim threshold 128 KB)
    proc_env["MALLOC_TRIM_THRESHOLD_"]  = "131072"
    proc_env["MALLOC_MMAP_THRESHOLD_"]  = "131072"
    # No .pyc files — saves disk I/O + slightly less RAM
    proc_env["PYTHONDONTWRITEBYTECODE"] = "1"
    # Use system malloc (returns memory to OS sooner than pymalloc)
    proc_env["PYTHONMALLOC"]            = "malloc"
    # Unbuffered output goes straight to log
    ...
    proc = await create_subprocess_exec(
        *parts,
        ...
        cwd=pdir,
        env=proc_env,
        start_new_session=True,  # Survives bot OOM kill
    )
    log_fd.close()
...
async def cb_run(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # RAM gate error — friendly message
        await safe_edit(
            query,
            f"\u26a0\ufe0f *Cannot Start \u2014 RAM Full*\n\n{str(_re)}\n\n"
            f"Stop another running project first, then retry.",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("\U0001f519 Back", callback_data=f"proj:{name}")]]),
            parse_mode=ParseMode.MARKDOWN,
        )
    except Exception as e:
...
# ─────────────────────────────────────────────────────────────
# Bot Settings — lock, maintenance (MongoDB "settings" collection)
# ─────────────────────────────────────────────────────────────

async def get_bot_settings() -> dict:
    doc = await settings_col.find_one({"_id": "bot_settings"})
    if not doc:
        return {"bot_locked": False, "maintenance_mode": False}
    return doc
...
async def is_bot_locked() -> bool:
    doc = await get_bot_settings()
    return bool(doc.get("bot_locked", False))
...
def rotate_log_if_needed(log_path: str):
    """Rotate log file if > 1 MB — prevents disk bloat on Render."""
    try:
        if os.path.exists(log_path) and os.path.getsize(log_path) > LOCAL_LOG_MAX_BYTES:
            bak = log_path + ".1"
...
async def cb_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if uid == OWNER_ID:
        ...
        lock_lbl  = "🔓 Unlock Bot"       if settings.get("bot_locked")       else "🔒 Lock Bot"
        maint_lbl = "✅ Disable Maint."   if settings.get("maintenance_mode") else "🔧 Maintenance"
        kb_rows.append([InlineKeyboardButton(lock_lbl,  callback_data="admin:toggle_lock"),
                         InlineKeyboardButton(maint_lbl, callback_data="admin:toggle_maintenance")])
        kb_rows.append([InlineKeyboardButton("🗄 DB Status",     callback_data="admin:db_status"),
                         InlineKeyboardButton("📊 Server Status", callback_data="admin:server_status")])
    kb_rows.append([InlineKeyboardButton("🔙 Back", callback_data="back_start")])
...
# ─────────────────────────────────────────────────────────────
# 🔒 Bot Lock / 🔧 Maintenance / 🗄 DB Status / 📊 Server Status
# ─────────────────────────────────────────────────────────────
...
async def cb_admin_toggle_lock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    settings = await get_bot_settings()
    new_val  = not settings.get("bot_locked", False)
    await set_bot_setting("bot_locked", new_val)
    if new_val:
        running = await projects_col.find(
            {"status": "running", "user_id": {"$ne": OWNER_ID}}
        ).to_list(length=500)
        for p in running:
        ...
        detail = f"Bot locked. {len(running)} non-owner project(s) stopped."
        label  = "🔒 Bot Locked"
    else:
        detail = "Bot unlocked. Users can start projects again."
        label  = "🔓 Bot Unlocked"
...
async def cb_admin_toggle_maintenance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    label  = "🔧 Maintenance ON"  if new_val else "✅ Maintenance OFF"
    detail = "Non-owner users see maintenance message." if new_val else "Bot open to all users."
    await safe_edit(query, f"✅ *{label}*\n\n{detail}",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")]]),
        parse_mode=ParseMode.MARKDOWN)
...
@owner_only
async def cb_admin_db_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        lines.append(f"🟢 Primary `{DATABASE_NAME}` — {ping_ms}ms — {count} projects")
    except Exception as e:
        lines.append(f"🔴 Primary `{DATABASE_NAME}` — ERROR: {str(e)[:60]}")
    for entry in extra_dbs:
        try:
            t0 = time.time()
            await entry["db"].command("ping")
            ping_ms = int((time.time() - t0) * 1000)
            count   = await entry["db"]["projects"].count_documents({})
            lines.append(f"🟢 Extra `{entry['name']}` — {ping_ms}ms — {count} projects")
        except Exception as e:
            lines.append(f"🔴 Extra `{entry['name']}` — ERROR: {str(e)[:50]}")
    lines.append(f"\n📊 Total DBs: `{1 + len(extra_dbs)}`")
    await safe_edit(query, "\n".join(lines),
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔃 Refresh", callback_data="admin:db_status"),
             InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")],
        ]),
        parse_mode=ParseMode.MARKDOWN)


async def _build_server_status_text() -> str:
    cpu  = psutil.cpu_percent(interval=0.3)
    ram  = psutil.virtual_memory()
    disk = psutil.disk_usage("/")
    swap = psutil.swap_memory()
    running_docs = await projects_col.find(
        {"status": "running"}, {"user_id": 1, "name": 1, "pid": 1}
    ).to_list(length=200)
    proj_lines     = []
    total_proj_ram = 0
    for doc in running_docs:
        ...
        if pid:
            try:
                ...
                total_proj_ram += rss
                proj_lines.append(f"   🟢 `{name}` — `{fmt_bytes(rss)}`")
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                proj_lines.append(f"   🔴 `{name}` — PID dead")
        else:
            proj_lines.append(f"   ⚪ `{name}` — no PID")
    ...
    lines = [
        ...
        f"│  Used `{fmt_bytes(ram.used)}`  Free `{fmt_bytes(ram.total - ram.used)}`  Total `{fmt_bytes(ram.total)}`",
        f"├ Swap: `{fmt_bytes(swap.used)}/{fmt_bytes(swap.total)}`" + (" ⚠️ active!" if swap.used > 0 else " ✅"),
        f"└ Disk: `{fmt_bytes(disk.used)}/{fmt_bytes(disk.total)}` (`{disk.percent}%`)",
        "",
    ]
    if proj_lines:
        lines.append(f"🗂 *Running Projects RAM* (`{len(proj_lines)}` running):")
        lines += proj_lines
        lines.append(f"📦 Projects total: `{fmt_bytes(total_proj_ram)}`")
        lines.append(f"🤖 Bot+OS overhead: `{fmt_bytes(max(0, ram.used - total_proj_ram))}`")
    else:
        lines.append("🗂 *Running Projects:* None")
    lines += [
        "",
        "⚠️ *Render Free: 512 MB RAM limit*",
        "🟢 Safe < 450 MB  |  🔴 OOM risk > 490 MB",
        f"⏰ Uptime: `{fmt_uptime(time.time() - BOT_START_TIME)}`",
    ]
    if ram.used > 490 * 1024 * 1024:
        lines.insert(0, "🚨 *WARNING: RAM critically high — OOM risk!*\n")
    return "\n".join(lines)


@owner_only
async def cb_admin_server_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    try:
        text = await _build_server_status_text()
    except Exception as e:
        text = f"📊 *Server Status*\n\n⚠️ Error: {str(e)[:200]}"
    await safe_edit(query, text,
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔃 Refresh",    callback_data="admin:server_status"),
             InlineKeyboardButton("🔙 Admin Panel", callback_data="admin_panel")],
        ]),
        parse_mode=ParseMode.MARKDOWN)
...
# ─────────────────────────────────────────────────────────────
# 🧹 RAM Cleanup Task (every 60s)
# ─────────────────────────────────────────────────────────────

async def ram_cleanup_task():
    """Every 60s: rotate logs, gc.collect, warn owner if RAM critical."""
    import gc
    _last_critical_alert = 0.0
    while True:
        ...
        try:
            ...
            total_proj_ram = 0
            for p in running:
                # Rotate log files — 10 projects × logs can eat disk fast
                log_path = os.path.join(project_dir(p["user_id"], p["name"]), "output.log")
                rotate_log_if_needed(log_path)
                # Accumulate project RAM
                _pid = p.get("pid")
                if _pid:
                    try:
                        total_proj_ram += psutil.Process(_pid).memory_info().rss
                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                        pass

            gc.collect()  # free cyclic garbage

            # Critical RAM alert to owner (at most once per 10 minutes)
            _ram = psutil.virtual_memory()
            _ram_mb = _ram.used / (1024 * 1024)
            if _ram_mb > RAM_CRITICAL_MB:
                _now = time.time()
                if _now - _last_critical_alert > 600:
                    _last_critical_alert = _now
                    logger.error(f"RAM critical: {_ram_mb:.0f} MB / {_ram.total//(1024*1024)} MB")
                    if notification_bot and OWNER_ID:
                        try:
                            await notification_bot.send_message(
                                chat_id=OWNER_ID,
                                text=(
                                    f"\U0001f6a8 *RAM Critical Alert*\n\n"
                                    f"Used: `{fmt_bytes(_ram.used)}` / `{fmt_bytes(_ram.total)}`"
                                    f" (`{_ram.percent}%`)\n"
                                    f"Projects RAM: `{fmt_bytes(total_proj_ram)}`\n"
                                    f"Running: `{len(running)}` projects\n\n"
                                    f"OOM watchdog will kill highest-RAM project if "
                                    f"RAM exceeds {RAM_WATCHDOG_MB} MB."
                                ),
                                parse_mode=ParseMode.MARKDOWN,
                            )
                        except Exception:
            ...
            logger.debug(f"ram_cleanup_task: {e}")

async def process_monitor():
    while True:
        await asyncio.sleep(30)
        try:
            running = await projects_col.find({"status": "running"}).to_list(length=200)

            # ── Layer 3: OOM Watchdog ───────────────────────────────────────
            ram      = psutil.virtual_memory()
            ram_mb   = ram.used / (1024 * 1024)
            if ram_mb > RAM_WATCHDOG_MB and running:
                # Find highest-RAM non-owner project and kill it
                victim = None
                max_rss = 0
                for _p in running:
                    _pid = _p.get("pid")
                    if not _pid or _p.get("user_id") == OWNER_ID:
                        continue
                    try:
                        _rss = psutil.Process(_pid).memory_info().rss
                        if _rss > max_rss:
                            max_rss = _rss
                            victim  = _p
                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                ...
                if victim:
                    ...
                    vuid  = victim["user_id"]
                    logger.warning(
                        f"OOM watchdog: killing {vuid}:{vname} "
                        f"({max_rss // (1024*1024)} MB) — total RAM {ram_mb:.0f} MB"
                    )
                    await kill_project(vuid, vname)
                    await projects_col.update_one(
                        {"user_id": vuid, "name": vname},
                        {"$set": {"status": "stopped", "pid": None,
                                  "admin_stopped": False,
                                  "oom_paused": True}},
                    )
                    ...
                    if notification_bot:
                        try:
                            await notification_bot.send_message(
                                chat_id=vuid,
                                text=(
                                    f"\u26a0\ufe0f *Memory Alert — Project Paused*\n\n"
                                    f"Project `{vname}` was using `{fmt_bytes(max_rss)}` RAM.\n"
                                    f"Server total: `{fmt_bytes(ram.used)}/{fmt_bytes(ram.total)}`\n\n"
                                    f"Paused automatically to prevent crash.\n"
                                    f"Start it again manually when RAM is free."
                                ),
                                parse_mode=ParseMode.MARKDOWN,
                            )
                        except Exception:
                    ...
                    if notification_bot and OWNER_ID:
                        try:
                            await notification_bot.send_message(
                                chat_id=OWNER_ID,
                                text=(
                                    f"\U0001f6a8 *OOM Watchdog Fired*\n\n"
                                    f"Killed `{vname}` (user `{vuid}`)\n"
                                    f"It used `{fmt_bytes(max_rss)}` RAM.\n"
                                    f"Total at kill: `{fmt_bytes(ram.used)}/{fmt_bytes(ram.total)}`"
                                ),
                                parse_mode=ParseMode.MARKDOWN,
                            )
                        except Exception:
    ...
    try:
        proc = await asyncio.wait_for(
            create_subprocess_exec(
                ...
                stdout=asyncio.subprocess.DEVNULL, stderr=PIPE, cwd=pdir,
            ),
            timeout=300,
        )
        _, stderr = await asyncio.wait_for(proc.communicate(), timeout=300)
        if proc.returncode == 0:
            # Layer 6: purge pip cache after install — free disk space immediately
            try:
                _purge = await asyncio.wait_for(
                    create_subprocess_exec(
                ...
                await asyncio.wait_for(_purge.communicate(), timeout=30)
            except Exception:
                pass
    ...
    try:
        ...
        for proj in running_projects:
            ...
            try:
                ...
                # Skip pip install if venv already exists — avoids OOM on 512 MB Render
                _venv_check = os.path.join(project_dir(uid, name), "venv")
                if not os.path.exists(_venv_check):
                    logger.info(f"Installing requirements for {uid}:{name}...")
                    success, msg = await _install_requirements_for_project(uid, name)
                    logger.info(f"Requirements for {uid}:{name}: {msg}")
                else:
                    success, msg = True, "venv exists, skipped pip on restart"
                    logger.info(f"Venv exists for {uid}:{name} — skipping pip install")

                # RAM check before starting — stagger if tight
                _ram = psutil.virtual_memory()
                _ram_mb = _ram.used / (1024 * 1024)
                if _ram_mb > RAM_SAFE_LIMIT_MB:
                    logger.warning(
                        f"Startup RAM gate: {_ram_mb:.0f} MB — waiting 15s before {name}"
                    )
                    await asyncio.sleep(15)
                else:
                    await asyncio.sleep(3)  # stagger: 3s between each start

                try:
                    updated = await start_project_process(uid, name)
                    logger.info(f"Auto-restarted: {uid}:{name} PID={updated.get('pid')}")
                except RuntimeError as ram_err:
                    # RAM gate refused start — mark as stopped, notify user
                    await projects_col.update_one(
                        {"user_id": uid, "name": name},
                        {"$set": {"status": "stopped", "pid": None}},
                    )
                    ...
                    if notification_bot:
                        try:
                            await notification_bot.send_message(
                                ...
                                text=(
                                    f"\u26a0\ufe0f *Project Delayed*\n\n"
                                    f"`{name}` could not start automatically — RAM too high.\n"
                                    f"Start it manually once other projects free up memory."
                                ),
                                parse_mode=ParseMode.MARKDOWN,
                            )
                        except Exception:
...
async def setup_venvs_background():
    """Create venvs for projects that don't have one yet.
    Sequential + RAM-gated — safe for 512 MB Render.
    """
    await asyncio.sleep(60)  # Let bot settle first
    try:
        all_projects = await projects_col.find({}).to_list(length=500)
        for proj in all_projects:
            uid  = proj["user_id"]
            name = proj["name"]
            pdir = project_dir(uid, name)
            ...
            if os.path.exists(pdir) and not os.path.exists(venv_dir):
                # RAM gate — wait if too tight
                _vram = psutil.virtual_memory()
                if _vram.used / (1024 * 1024) > RAM_SAFE_LIMIT_MB:
                    logger.info(f"setup_venvs: RAM high, waiting 30s before {name}")
                    await asyncio.sleep(30)
                try:
                    proc = await create_subprocess_exec(
                        sys.executable, "-m", "venv", venv_dir,
                    ...
                    if os.path.exists(req_file) and os.path.exists(pip_path):
                        proc2 = await create_subprocess_exec(
                            pip_path, "install", "--no-cache-dir", "-r", req_file,
                            stdout=asyncio.subprocess.DEVNULL,
                            stderr=asyncio.subprocess.DEVNULL, cwd=pdir
                        )
...
def build_application() -> Application:
    ...
    app.add_handler(CallbackQueryHandler(cb_reinstall_reqs,          pattern=r"^reinstall_reqs_force:"))
    ...
    app.add_handler(CallbackQueryHandler(cb_admin_db_status,          pattern="^admin:db_status$"))
    app.add_handler(CallbackQueryHandler(cb_admin_server_status,      pattern="^admin:server_status$"))

    return app


async def post_init(app: Application):
    global notification_bot, INSTALL_SEMAPHORE
    notification_bot  = app.bot
    # Semaphore MUST be created inside the running event loop
    # Max 2 concurrent pip installs — safe for 512 MB Render
    INSTALL_SEMAPHORE  = asyncio.Semaphore(2)   # max 2 concurrent pip installs
    STARTUP_SEMAPHORE  = asyncio.Semaphore(1)   # max 1 project starts at a time
    # Tell glibc to return freed memory to OS faster (bot process itself)
    os.environ.setdefault("MALLOC_TRIM_THRESHOLD_", "131072")
    os.environ.setdefault("MALLOC_MMAP_THRESHOLD_", "131072")

    await app.bot.set_my_commands([
        BotCommand("start",  "Open main menu"),
        BotCommand("status", "Server RAM/CPU status (owner only)"),
    ...
    asyncio.create_task(ram_cleanup_task())
    asyncio.create_task(auto_restart_on_startup())
    asyncio.create_task(setup_venvs_background())
