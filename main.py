...
async def start_project_process(uid: int, name: str) -> dict:
    """Start project subprocess with isolated environment. Returns updated project dict."""
    p = await get_project(uid, name)
    ...
    # Only include essential system vars, NOT hosting bot vars
    SYSTEM_VARS = {"PATH", "HOME", "USER", "LANG", "LC_ALL", "TERM", "SHELL",
                   "TMPDIR", "TMP", "TEMP", "PYTHONPATH", "VIRTUAL_ENV",
                   "LD_LIBRARY_PATH", "C_INCLUDE_PATH", "PKG_CONFIG_PATH"}
    ...
    venv_bin = os.path.join(pdir, "venv", "bin")
    if os.path.exists(venv_bin):
        clean_env["PATH"] = venv_bin + ":" + clean_env.get("PATH", "/usr/bin:/bin")
        clean_env["VIRTUAL_ENV"] = os.path.join(pdir, "venv")

    # Load project's own .env file into the clean environment
    ...
    if os.path.exists(env_path):
        try:
            ...
            logger.warning(f"Failed to load .env for {name}: {e}")

    # Set PORT to avoid conflict with hosting bot's Flask
    if "PORT" not in clean_env:
        clean_env["PORT"] = "0"  # Random port or disabled
    ...
    proc = await create_subprocess_exec(
        *parts,
        ...
        stderr=log_fd,
        cwd=pdir,
        env=clean_env,  # USE ISOLATED ENVIRONMENT
    )

    logger.info(f"Process started with PID {proc.pid} (isolated env)")
