...
def project_dashboard_kb(user_id: int, project_name: str, auto_restart: bool = True, is_running: bool = False, is_locked: bool = False) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        row1,
        [
            InlineKeyboardButton("📺 Live Logs", callback_data=f"live_logs:{pn}"),
            InlineKeyboardButton("📊 Uptime",    callback_data=f"uptime:{pn}"),
        ...
        [
            ...
            InlineKeyboardButton("🔁 Clone",     callback_data=f"clone:{pn}"),
        ],
        [
        ...
        [
            ...
            InlineKeyboardButton("⏰ Cron Jobs", callback_data=f"cron:{pn}"),
            InlineKeyboardButton("🔔 Notifs",    callback_data=f"notif:{pn}"),
        ],
        [
        ...
        try:
            ...
            for p in running:
                ...
                if pid and not psutil.pid_exists(pid):
                    ...
                    # Auto-restart logic
                    # FIX: check code is not None — killed processes have code=None, None != 0 is True in Python
                    ...
                    # FIX: same None guard for crash notification branch
...
async def keep_alive_task():
    """Ping health endpoint every 2 minutes, clean expired tokens."""
    import urllib.request
    health_url = f"{BASE_URL}/health"
    logger.info(f"Keep-alive task started. Pinging {health_url} every 2 minutes.")

    while True:
        # FIX: ping every 2 minutes (was 10 minutes) to keep the server alive
        ...
        try:
            # FIX: use get_running_loop() — get_event_loop() is deprecated in async context
            loop = asyncio.get_running_loop()
    ...
    try:
        ...
        for proj in running_projects:
            ...
            try:
                ...
                # GitHub: git pull before starting if URL is set
                    ...
                    try:
                        ...
                        if os.path.exists(git_dir):
                            gp = await asyncio.wait_for(
                                create_subprocess_exec("git", "-C", pdir, "pull", "--rebase", stdout=PIPE, stderr=PIPE),
                                timeout=60,
                            )
                            await asyncio.wait_for(gp.communicate(), timeout=60)
                            logger.info(f"GitHub pull on startup for {uid}:{name}: code={gp.returncode}")
                    except Exception as ge:
                        logger.warning(f"GitHub pull on startup failed for {uid}:{name}: {ge}")
    ...
    try:
        ...
        if os.path.exists(git_dir):
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                proc = await asyncio.wait_for(
                    create_subprocess_exec("git", "clone", "--depth=1", github_url, tmpdir, stdout=PIPE, stderr=PIPE),
                    timeout=180,
                )
...
async def clone_name_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not _re.match(r"^[a-zA-Z0-9_]{1,20}$", new_name):
    ...
    try:
        ...
        _shutil.copytree(
            src_dir, dst_dir,
            ignore=_shutil.ignore_patterns("venv", "__pycache__", "*.pyc", "output.log"),
        )
        await projects_col.insert_one({
