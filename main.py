...
# Migration lock — process_monitor pauses while migration is in progress
MIGRATION_IN_PROGRESS = False

# Semaphore: max 2 concurrent pip/npm installs (safe for Render 512 MB plan)
INSTALL_SEMAPHORE: asyncio.Semaphore = None  # initialized in post_init

# Primary MongoDB
mongo_client = AsyncIOMotorClient(MONGODB_URI, maxPoolSize=5, minPoolSize=1, serverSelectionTimeoutMS=10000)
...
def _load_extra_databases():
    ...
    if legacy_uri and legacy_name and legacy_name not in seen_names:
        try:
            client = AsyncIOMotorClient(legacy_uri, maxPoolSize=3, minPoolSize=1)
    ...
    for i in range(1, 51):
        ...
        if not uri or not name or name in seen_names:
            continue
        try:
            client = AsyncIOMotorClient(uri, maxPoolSize=3, minPoolSize=1)
            extra_clients.append(client)
...
async def migrate_mongo_to_local() -> tuple:
    ...
    Sets MIGRATION_IN_PROGRESS so process_monitor skips crash-detection during migration."""
    ...
    try:
        ...
        # Process users in batches of 500 to avoid RAM spike
        BATCH = 500
        skip = 0
        while True:
            batch = await users_col.find({}).skip(skip).limit(BATCH).to_list(length=BATCH)
            if not batch:
                break
            def _write_users(users):
                ...
                conn.close()
                return len(users)
            uc += await loop.run_in_executor(None, _write_users, batch)
            skip += BATCH
            await asyncio.sleep(0)  # yield to event loop between batches

        # Process projects in batches of 500
        skip = 0
        while True:
            batch = await projects_col.find({}).skip(skip).limit(BATCH).to_list(length=BATCH)
            if not batch:
                break
            def _write_projects(projects):
                ...
                conn.close()
                return len(projects)
            pc += await loop.run_in_executor(None, _write_projects, batch)
            skip += BATCH
            await asyncio.sleep(0)  # yield to event loop between batches
...
MAX_LOG_SIZE  = 512 * 1024   # rotate when log exceeds 512 KB
MAX_LOG_KEEP  = 200 * 1024   # keep only last 200 KB after rotation

def rotate_log_if_needed(log_path: str):
    """If log > 512 KB, keep only the last 200 KB."""
    try:
        if os.path.exists(log_path) and os.path.getsize(log_path) > MAX_LOG_SIZE:
            with open(log_path, "rb") as f:
                f.seek(-MAX_LOG_KEEP, 2)
                data = f.read()
...
async def cb_reinstall_reqs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.answer()
    uid  = query.from_user.id
    # Supports both 'reinstall_reqs:name' and 'reinstall_reqs_force:name'
    raw   = query.data
    ...
    # Warn user if the project is currently running — pip install can cause OOM on 512 MB Render
    if p.get("status") == "running" and not force:
        await safe_edit(
            query,
            f"⚠️ *{escape_md(name)} is currently running!*\n\n"
            f"Installing requirements while a project is running may cause it to crash due to RAM limits.\n\n"
            f"👉 *Recommended:* Stop the project first, then reinstall.\n\n"
    ...
    # Acquire semaphore — parallel installs allowed, no user-facing wait messages
    sem = INSTALL_SEMAPHORE or asyncio.Semaphore(2)
    async with sem:
        ...
        await req_progress.start("pip install --no-cache-dir -r requirements.txt")
        req_progress.run_in_background(estimated_seconds=120, status="Resolving + downloading wheels")
        try:
            proc = await asyncio.wait_for(
                create_subprocess_exec(pip_path, "install", "--no-cache-dir", "-r", req_path, "--upgrade",
                                       stdout=PIPE, stderr=PIPE, cwd=pdir),
                timeout=600,
            )
...
async def _finalize_new_project(update: Update, context: ContextTypes.DEFAULT_TYPE, via_message: bool):
    ...
    if os.path.exists(req_path) and os.path.exists(pip_path):
        ...
        try:
            proc = await asyncio.wait_for(
                create_subprocess_exec(pip_path, "install", "--no-cache-dir", "-r", req_path,
                                       stdout=PIPE, stderr=PIPE, cwd=pdir),
                timeout=300,
            )
...
# ─────────────────────────────────────────────────────────────
# 📊 Server Status (Owner only)
# ─────────────────────────────────────────────────────────────

async def _build_server_status_text() -> str:
    """Build the server status message — shared by button & /status command."""
    cpu    = psutil.cpu_percent(interval=0.3)
    ram    = psutil.virtual_memory()
    disk   = psutil.disk_usage("/")
    swap   = psutil.swap_memory()

    # Per-project RAM breakdown
    ...
    lines = [
        ...
        f"│  Used:  `{fmt_bytes(ram.used)}`  Free: `{fmt_bytes(ram_free)}`  Total: `{fmt_bytes(ram.total)}`",
        f"├ Swap: `{fmt_bytes(swap.used)}/{fmt_bytes(swap.total)}`" + (" ⚠️ swap active!" if swap.used > 0 else " ✅"),
        f"└ Disk: `{fmt_bytes(disk.used)}/{fmt_bytes(disk.total)}` (`{disk.percent}%`)",
        "",
    ]

    if proj_lines:
        lines.append(f"🗂 *Running Projects RAM* (`{len(proj_lines)}` projects):")
        lines += proj_lines
        lines.append(f"📦 Total by projects: `{fmt_bytes(total_proj_ram)}`")
        bot_ram = fmt_bytes(ram.used - total_proj_ram) if ram.used > total_proj_ram else "~"
        lines.append(f"🤖 Bot overhead: `{bot_ram}`")
    else:
        lines.append("🗂 *Running Projects:* None")

    lines += [
        "",
        f"⚠️ *Render Free Plan limit: 512 MB RAM*",
        f"🟢 Safe below 450 MB  |  🔴 OOM risk above 490 MB",
    ]

    danger = ram.used > 490 * 1024 * 1024
    if danger:
...
async def cb_admin_server_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Callback: 📊 Server Status button in Admin Panel (owner only)."""
    query = update.callback_query
    await query.answer()
    try:
...
async def cb_admin_backup_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        async for proj in projects_col.find({}):  # stream — avoid loading all into RAM
            uid  = proj["user_id"]
            name = proj["name"]
            pdir = project_dir(uid, name)
        ...
        try:
            ...
            await backups_col.insert_one({
                "type": "backup_meta", "total_projects": all_projects_count,
...
async def cb_admin_all_projects(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    all_projects = await projects_col.find(
        {}, {"user_id": 1, "name": 1, "status": 1, "locked": 1}  # fetch only needed fields
    ).to_list(length=500)  # cap at 500 — was 10000, causing OOM on Render free plan
    user_projects = {}
...
async def admin_broadcast_msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    bot  = update.get_bot()
    sent = failed = 0
    async for u in users_col.find({}):  # stream — never load 10000 users into RAM at once
        try:
...
async def process_monitor():
    while True:
        ...
        try:
            # Skip crash-detection while DB migration is running to avoid false restarts
            if MIGRATION_IN_PROGRESS:
                logger.info("process_monitor: migration in progress, skipping cycle")
                continue
            running = await projects_col.find({"status": "running"}).to_list(length=100)
            for p in running:
                pid = p.get("pid")
...
async def backup_task():
    """
    RAM-optimised backup: processes ONE project at a time (never loads all into RAM),
    yields to event loop between projects, and runs GC after each batch.
    Render free plan has only 512 MB — loading 10 000 projects at once OOMs the process.
    """
    while True:
        await asyncio.sleep(600)  # every 10 min — was 5 min, reduced to ease memory pressure
        try:
            db_distribution = {}
            total_files = 0
            total_size  = 0
            ...
            # Use cursor — never load all projects into RAM at once
            ...
            async for proj in cursor:
                ...
                for root, dirs, files in os.walk(pdir):
                    ...
                    for fname in files:
                        ...
                        try:
                            file_size = os.path.getsize(fpath)
                            if file_size > 5 * 1024 * 1024:  # skip files >5 MB (was 15 MB)
                                continue
                            try:
                                with open(fpath, "r", encoding="utf-8") as f:
                                    content = f.read()
                ...
                total_projects += 1
                files_data.clear()   # free memory immediately
                del files_data
                gc.collect()         # force GC after each project
                await asyncio.sleep(0)  # yield to event loop between projects

            await backups_col.delete_many({"type": "backup_meta"})
            await backups_col.insert_one({
                "type": "backup_meta", "total_projects": total_projects,
                "total_files": total_files, "total_size": total_size,
                "backed_up_at": datetime.now(timezone.utc),
                "distribution": db_distribution,
            })
            logger.info(f"Auto backup: {total_projects} projects, {total_files} files — distribution: {db_distribution}")
...
async def ram_cleanup_task():
    ...
    while True:
        ...
        try:
            ...
            # Log current RAM usage so we can monitor Render free plan headroom
            try:
                proc_mem = psutil.Process().memory_info()
                rss_mb = proc_mem.rss / 1024 / 1024
                logger.info(f"RAM usage: {rss_mb:.1f} MB RSS")
            except Exception:
                pass
...
async def keep_alive_task():
    """Ping health endpoint every 2 minutes to prevent Render sleep + clean expired tokens."""
    import urllib.request
    health_url = f"{BASE_URL}/health"
    logger.info(f"Keep-alive task started. Pinging {health_url} every 2 minutes.")
...
async def _install_requirements_for_project(uid: int, name: str) -> tuple:
    """Background install — uses semaphore (max 2 concurrent) and discards output to save RAM."""
    pdir     = project_dir(uid, name)
    req_path = os.path.join(pdir, "requirements.txt")
    pkg_json = os.path.join(pdir, "package.json")
    venv_dir = os.path.join(pdir, "venv")
    ...
    async with sem:
        if os.path.exists(pkg_json) and not os.path.exists(req_path):
            try:
                proc = await asyncio.wait_for(
                    create_subprocess_exec("npm", "install", "--no-audit", "--no-fund",
                                           stdout=devnull, stderr=PIPE, cwd=pdir),
                    timeout=300,
                )
                ...
                if proc.returncode == 0:
                    ...
                    return (False, f"npm install failed: {(stderr or b'').decode()[:200]}")
            except asyncio.TimeoutError:
        ...
        if not os.path.exists(pip_path):
            try:
                ...
                _, stderr = await asyncio.wait_for(proc.communicate(), timeout=120)
                if proc.returncode != 0:
                    return (False, f"venv create failed: {(stderr or b'').decode()[:200]}")
            except Exception as e:
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                return (False, f"pip install failed: {(stderr or b'').decode()[:300]}")
        except asyncio.TimeoutError:
            return (False, "pip install timed out")
        except Exception as e:
            return (False, f"pip error: {e}")

async def _startup_restart_one(proj: dict, bot_locked: bool) -> None:
    """Helper: install deps + start one project on startup. Called sequentially from auto_restart_on_startup."""
    uid  = proj["user_id"]
    name = proj["name"]
    try:
        if bot_locked and uid != OWNER_ID:
            ...
            if not user_prem:
                ...
                logger.info(f"Skipped {uid}:{name} — bot locked, free user")
                return
        ...
        if notification_bot:
            try:
                await notification_bot.send_message(
                    ...
                    text=(
                        f"🔄 *Bot Restarted*\n\n"
                        f"Project `{name}` is being set up...\n"
                        f"▶️ Starting automatically!"
                    ),
                    parse_mode=ParseMode.MARKDOWN,
                )
            except Exception:
        ...
        # Only install requirements if venv is missing — skip if already installed.
        # Running pip install on every bot restart causes OOM on 512 MB Render plan.
        venv_dir_check = os.path.join(project_dir(uid, name), "venv")
        if not os.path.exists(venv_dir_check):
            success, msg = await _install_requirements_for_project(uid, name)
            logger.info(f"Requirements for {uid}:{name}: {msg}")
        else:
            success, msg = True, "venv exists, skipped reinstall"
            logger.info(f"Venv already exists for {uid}:{name} — skipping pip install on startup")
        ...
        if notification_bot:
            try:
                req_status = "✅ Requirements ready" if success else f"⚠️ {msg[:80]}"
                await notification_bot.send_message(
                    chat_id=uid,
                    text=(
                        f"✅ *Project Started*\n\n"
        ...
        logger.error(f"Startup restart failed for {uid}:{name}: {e}")
        try:
            await projects_col.update_one(
                {"user_id": uid, "name": name},
                {"$set": {"status": "stopped", "pid": None}},
...
async def auto_restart_on_startup():
    """Restart all running projects sequentially on bot startup. Respects bot_lock & maintenance."""
    await asyncio.sleep(30)  # wait for files to restore first (old-style: 30s)
    try:
        ...
        running_projects = await projects_col.find({
            ...
            "locked":        {"$ne": True},
        }).to_list(length=200)  # cap at 200 — Render free plan can't handle thousands

        if not running_projects:
            logger.info("Auto-restart on startup: no running projects found.")
            return

        logger.info(f"Auto-restart on startup: {len(running_projects)} projects (sequential, old-style)...")
        bot_locked = await is_bot_locked()

        # SEQUENTIAL loop — OLD script jaisa, ek ek project start karo
        # Render free plan 512 MB: parallel installs = instant OOM crash
        for proj in running_projects:
            await _startup_restart_one(proj, bot_locked)
            gc.collect()
            await asyncio.sleep(2)  # small pause between each project
...
async def _setup_venv_one(proj: dict) -> None:
    """Create venv + install requirements for one project. Called in parallel via gather."""
    uid      = proj["user_id"]
    name     = proj["name"]
    pdir     = project_dir(uid, name)
    venv_dir = os.path.join(pdir, "venv")
    ...
    if not (os.path.exists(pdir) and not os.path.exists(venv_dir)):
        return
        ...
        try:
            ...
            if os.path.exists(req_file) and os.path.exists(pip_path):
                proc2 = await create_subprocess_exec(
                    ...
                    stdout=devnull, stderr=devnull, cwd=pdir
                )
...
async def setup_venvs_background():
    """
    Startup pe missing venvs banao — OLD script jaisa SEQUENTIAL (ek ek karke).
    Render free plan (512 MB): parallel installs = OOM crash, isliye ek ek karo.
    """
    await asyncio.sleep(5)  # let bot finish initialising first
    try:
        all_projects = await projects_col.find(
            {}, {"user_id": 1, "name": 1}
        ).to_list(length=500)
        logger.info(f"setup_venvs_background: {len(all_projects)} projects (sequential)...")
        devnull = asyncio.subprocess.DEVNULL
        for proj in all_projects:
            ...
            try:
                ...
                logger.warning(f"Failed to setup venv for {name}: {e}")
            gc.collect()
            await asyncio.sleep(1)  # yield + small pause between each install
        logger.info("setup_venvs_background: done.")
    except Exception as e:
...
async def post_init(app: Application):
    ...
    # 2 concurrent installs — safe for Render free plan (512 MB RAM)
