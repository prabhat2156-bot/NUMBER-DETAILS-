...
async def cb_my_projects(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # OPT: projection — my_projects only needs name+status+pid for buttons
    projects = await projects_col.find(
        {"user_id": uid}, {"name":1,"status":1,"pid":1,"run_command":1,"_id":0}
    ).to_list(length=100)
    if not projects:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="back_start")]])
...
async def new_project_github_url(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    progress = LiveProgress(update.message, title=f"GitHub Clone — {name}")
...
async def cb_admin_running(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # OPT: projection — cb_admin_running only needs uid/name/cmd/pid
        running = await projects_col.find(
            {"status": "running"},
            {"user_id":1,"name":1,"run_command":1,"pid":1,"_id":0}
        ).to_list(length=100)
        if not running:
            await safe_edit(
                query,
...
async def cb_admin_all_projects(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # OPT: projection — admin_all_projects only needs uid/name/status/cmd for display
    all_projects = await projects_col.find(
        {}, {"user_id":1,"name":1,"status":1,"run_command":1,"_id":0}
    ).to_list(length=10000)
    user_projects = {}
    ...
    while True:
        ...
        try:
            ...
            _mon_proj = {"user_id":1,"name":1,"pid":1,"auto_restart":1,"run_command":1,
                         ...
                         "started_at":1,"notif_crash":1,"notif_restart":1,"_id":0}
...
async def auto_restart_on_startup():
    ...
    try:
        ...
        # OPT: projection — only fields auto_restart_on_startup needs
        _ar_proj = {"user_id":1,"name":1,"github_url":1,"github_last_pull":1,
                    "run_command":1,"auto_restart":1,"_id":0}
