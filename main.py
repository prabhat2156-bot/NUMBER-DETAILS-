...
# ─────────────────────────────────────────────────────────────
# Version selection — supported runtimes
# ─────────────────────────────────────────────────────────────
...
def detect_available_python_versions() -> list:
    """Return list of python versions available on host (e.g. ['3.10','3.11'])."""
    found = []
...
def detect_available_node_versions() -> list:
    """Return list of node versions available on host."""
    found = []
    for v in SUPPORTED_NODE_VERSIONS:
        # try common naming patterns
        if shutil.which(f"node{v}") or shutil.which(f"node-{v}") or shutil.which(f"nodejs{v}"):
            found.append(v)
    return found


def python_binary_for(version: str | None) -> str:
    """Return python executable path for given version, fallback to sys.executable."""
    if version:
        b = shutil.which(f"python{version}")
        if b:
            return b
    return sys.executable


def node_binary_for(version: str | None) -> str:
    """Return node executable for given version, fallback to 'node'."""
    if version:
        for cand in (f"node{version}", f"node-{version}", f"nodejs{version}"):
            b = shutil.which(cand)
            if b:
                return b
...
def npm_binary_for(version: str | None) -> str:
    """Return npm executable for given node version, fallback to 'npm'."""
    if version:
        for cand in (f"npm{version}", f"npm-{version}"):
            b = shutil.which(cand)
            if b:
                return b
...
# ─────────────────────────────────────────────────────────────
# 💖 Love Animation — non-stop heart animation during long ops
# ─────────────────────────────────────────────────────────────
PROGRESS_TOTAL = 20  # total bar segments

class LoveAnimator:
    """Background asyncio task that edits a Telegram message with a non-stop
    progress-bar animation while a long-running setup step is in progress.
    The bar fills 0% → 100% then loops back to 0% continuously until stop()."""

    def __init__(self, message, header: str, interval: float = 1.2):
    ...
    def _render(self, idx: int) -> str:
        # Cycle through 1..PROGRESS_TOTAL then loop
        step_in_cycle = (idx % PROGRESS_TOTAL) + 1
        percent = int(step_in_cycle * 100 / PROGRESS_TOTAL)
        filled  = "▓" * step_in_cycle
        empty   = "░" * (PROGRESS_TOTAL - step_in_cycle)
        body    = "\n".join(self.results) if self.results else ""
        return (
            f"{self.header}\n\n"
            + (f"{body}\n\n" if body else "")
            + f"⏳ *{escape_md_safe(self.current_step)}*\n"
    ...
    async def _loop(self):
        ...
        while not self._stop:
            text = self._render(idx)
            try:
                await self.message.edit_text(text, parse_mode=ParseMode.MARKDOWN)
            except BadRequest:
                # message not modified or parse error — ignore
                pass
            except Exception as e:
                logger.debug(f"LoveAnimator edit error: {e}")
            try:
    ...
    async def stop(self, final_text: str | None = None, reply_markup=None):
        self._stop = True
        if self._task:
            self._task.cancel()
            try:
        ...
        if final_text is not None:
            try:
                await self.message.edit_text(
                    final_text, reply_markup=reply_markup,
...
def escape_md_safe(text: str) -> str:
    """Lightweight markdown-safe escaping (for use before escape_md is defined)."""
    if not text:
        return ""
...
async def cb_reinstall_reqs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Resolve runtimes from project doc
    py_ver   = p.get("python_version")
    node_ver = p.get("node_version")
    py_bin   = python_binary_for(py_ver)
    npm_bin  = npm_binary_for(node_ver)
    py_label   = (f"Python {py_ver}"   if py_ver   else "Python (system default)")
    node_label = (f"Node.js {node_ver}" if node_ver else "Node.js (system default)")

    # Node.js project — run `npm install` with love animation
    if os.path.exists(pkg_json) and not os.path.exists(req_path):
        status_msg = await query.message.reply_text(
            f"💖 *Reinstalling npm for {escape_md(name)}* 💖\n\nStarting...",
            parse_mode=ParseMode.MARKDOWN,
        )
        animator = LoveAnimator(
            status_msg,
            header=f"💖 *Reinstalling npm for {escape_md(name)}* 💖\n🟢 {escape_md(node_label)}",
        )
        animator.start()
        animator.set_step(f"📦 Installing npm packages with {node_label}...")
        try:
            ...
            if proc_n.returncode == 0:
                msg = f"💖🎉 *npm packages reinstalled for {escape_md(name)}!* 🎉💖\n🟢 {escape_md(node_label)}"
            else:
                err = (stderr_n or b"").decode()[:400]
                msg = f"❌ *npm install failed for {escape_md(name)}*\n\n```\n{err}\n```"
        except asyncio.TimeoutError:
            msg = f"❌ *npm install timed out for {escape_md(name)}*"
        except FileNotFoundError:
            msg = f"❌ npm not installed for {escape_md(node_label)}."
        except Exception as e:
            msg = f"❌ npm error: {escape_md(str(e))}"

        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("🔄 Restart Project", callback_data=f"restart:{name}")],
            [InlineKeyboardButton("🔙 Back", callback_data=f"proj:{name}")],
        ])
        await animator.stop(final_text=msg, reply_markup=kb)
        return
    ...
    # Python flow with love animation
    status_msg = await query.message.reply_text(
        f"💖 *Reinstalling requirements for {escape_md(name)}* 💖\n\nStarting...",
        parse_mode=ParseMode.MARKDOWN,
    )
    animator = LoveAnimator(
        status_msg,
        header=f"💖 *Reinstalling requirements for {escape_md(name)}* 💖\n🐍 {escape_md(py_label)}",
    )
    animator.start()

    # Ensure venv exists
    if not os.path.exists(pip_path):
        animator.set_step(f"📁 Creating virtual environment with {py_label}...")
        try:
            proc = await asyncio.wait_for(
                create_subprocess_exec(py_bin, "-m", "venv", venv_dir,
                                       stdout=PIPE, stderr=PIPE),
                timeout=120,
            )
            _, stderr = await asyncio.wait_for(proc.communicate(), timeout=120)
            if proc.returncode == 0:
                animator.add_result(f"✅ Virtual environment created ({py_label})")
            else:
                animator.add_result(f"❌ venv failed: {stderr.decode()[:200]}")
                await animator.stop(
                    final_text=f"📦 *Reinstall failed*\n\n" + "\n".join(animator.results),
                    reply_markup=back_kb,
                )
                return
        except Exception as e:
            animator.add_result(f"❌ venv error: {e}")
            await animator.stop(
                final_text=f"📦 *Reinstall failed*\n\n" + "\n".join(animator.results),
                reply_markup=back_kb,
            )
    ...
    animator.set_step("⬆️ Upgrading pip...")
    try:
        proc = await asyncio.wait_for(
        ...
        animator.add_result("✅ pip upgraded")
    except Exception:
        animator.add_result("⚠️ pip upgrade skipped")

    # Install requirements (force reinstall to pick up changes)
    animator.set_step("📦 Installing requirements (this may take a while)...")
    try:
        proc = await asyncio.wait_for(
            create_subprocess_exec(
        ...
        if proc.returncode == 0:
            animator.add_result("✅ Requirements installed successfully")
        else:
            err = stderr.decode()[:400] if stderr else "unknown error"
            animator.add_result(f"❌ pip install failed:\n```\n{err}\n```")
            await animator.stop(
                final_text=f"📦 *Reinstall failed for {name}*\n\n" + "\n".join(animator.results),
                reply_markup=back_kb,
            )
            return
    except asyncio.TimeoutError:
        animator.add_result("❌ pip install timed out")
        await animator.stop(
            final_text=f"📦 *Reinstall failed for {name}*\n\n" + "\n".join(animator.results),
            reply_markup=back_kb,
        )
        ...
        animator.add_result(f"❌ pip error: {e}")
        await animator.stop(
            final_text=f"📦 *Reinstall failed for {name}*\n\n" + "\n".join(animator.results),
            reply_markup=back_kb,
        )
    ...
    # Verify
    animator.set_step("🔍 Verifying packages...")
    try:
        ...
        animator.add_result(f"✅ {pkg_count} packages available")
    except Exception:
        animator.add_result("⚠️ Could not verify packages")
    ...
    await animator.stop(final_text=final, reply_markup=kb)
...
async def start_project_process(uid: int, name: str) -> dict:
    ...
    if not os.path.exists(venv_python):
        venv_python = python_binary_for(py_ver)
    ...
    if parts and parts[0] in ("python", "python3"):
        ...
        parts[0] = node_binary_for(node_ver)
    elif parts and parts[0] == "npm":
        parts[0] = npm_binary_for(node_ver)

    logger.info(f"Starting process: {' '.join(parts)} in {pdir} (py={py_ver}, node={node_ver})")
...
async def new_project_done_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    return await _ask_version(update, context, via_message=True)

async def new_project_done_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    return await _ask_version(update, context, via_message=False)
...
def _project_kinds(pdir: str) -> tuple[bool, bool]:
    """Return (has_python, has_node) by inspecting uploaded files."""
    has_python = (
        os.path.exists(os.path.join(pdir, "requirements.txt"))
        or any(f.endswith(".py") for f in os.listdir(pdir) if os.path.isfile(os.path.join(pdir, f)))
    )
    has_node = os.path.exists(os.path.join(pdir, "package.json")) or any(
        f.endswith(".js") for f in os.listdir(pdir) if os.path.isfile(os.path.join(pdir, f))
    )
    return has_python, has_node


def _version_keyboard(has_python: bool, has_node: bool, py_sel: str | None, node_sel: str | None) -> InlineKeyboardMarkup:
    rows = []
    if has_python or not has_node:
        py_avail = detect_available_python_versions()
        rows.append([InlineKeyboardButton("🐍 Python Version:", callback_data="ver_noop")])
        # show in rows of 3
        line = []
        for v in SUPPORTED_PYTHON_VERSIONS:
            mark = "✅ " if py_sel == v else ("🔘 " if v in py_avail else "⚪ ")
            line.append(InlineKeyboardButton(f"{mark}{v}", callback_data=f"ver_py:{v}"))
            if len(line) == 3:
                rows.append(line); line = []
        if line:
            rows.append(line)
        rows.append([InlineKeyboardButton(
            ("✅ Default (system)" if py_sel == "default" else "🔘 Default (system)"),
            callback_data="ver_py:default")])

    if has_node:
        node_avail = detect_available_node_versions()
        rows.append([InlineKeyboardButton("🟢 Node.js Version:", callback_data="ver_noop")])
        line = []
        for v in SUPPORTED_NODE_VERSIONS:
            mark = "✅ " if node_sel == v else ("🔘 " if v in node_avail else "⚪ ")
            line.append(InlineKeyboardButton(f"{mark}{v}", callback_data=f"ver_node:{v}"))
            if len(line) == 3:
                rows.append(line); line = []
        if line:
            rows.append(line)
        rows.append([InlineKeyboardButton(
            ("✅ Default (system)" if node_sel == "default" else "🔘 Default (system)"),
            callback_data="ver_node:default")])

    rows.append([InlineKeyboardButton("🚀 Confirm & Setup", callback_data="ver_confirm")])
    rows.append([InlineKeyboardButton("❌ Cancel", callback_data="back_start")])
    return InlineKeyboardMarkup(rows)


async def _ask_version(update: Update, context: ContextTypes.DEFAULT_TYPE, via_message: bool):
    uid  = update.effective_user.id
    name = context.user_data.get("new_project_name")
    pdir = project_dir(uid, name)
    ...
    has_python, has_node = _project_kinds(pdir)
    if not has_python and not has_node:
        # default to python so user can still pick
        has_python = True

    # default selections
    py_avail   = detect_available_python_versions()
    node_avail = detect_available_node_versions()
    context.user_data["sel_python_version"] = py_avail[-1] if py_avail else "default"
    context.user_data["sel_node_version"]   = node_avail[-1] if node_avail else "default"
    context.user_data["has_python"] = has_python
    context.user_data["has_node"]   = has_node

    text = (
        f"⚙️ *Choose Runtime Version for {escape_md(name)}*\n\n"
        f"Select the version you want to use to run your project.\n"
        f"_✅ = selected, 🔘 = available on host, ⚪ = not installed_\n\n"
        f"Latest available version is auto-selected. Tap to change."
    )
    kb = _version_keyboard(
        has_python, has_node,
        context.user_data["sel_python_version"],
        context.user_data["sel_node_version"],
    )
    target = update.message if via_message else update.callback_query.message
    await target.reply_text(text, reply_markup=kb, parse_mode=ParseMode.MARKDOWN)
    return NEW_PROJECT_VERSION


async def cb_version_pick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    data = query.data
    if data == "ver_noop":
    ...
    if data.startswith("ver_py:"):
        context.user_data["sel_python_version"] = data.split(":", 1)[1]
    elif data.startswith("ver_node:"):
        context.user_data["sel_node_version"] = data.split(":", 1)[1]

    has_python = context.user_data.get("has_python", True)
    has_node   = context.user_data.get("has_node", False)
    name       = context.user_data.get("new_project_name", "")
    ...
    kb = _version_keyboard(
        has_python, has_node,
        context.user_data.get("sel_python_version"),
        context.user_data.get("sel_node_version"),
    )
    try:
...
async def cb_version_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
...
async def _finalize_new_project(update: Update, context: ContextTypes.DEFAULT_TYPE, via_message: bool):
    ...
    # Selected versions (None or "default" => system default)
    py_ver_sel   = context.user_data.get("sel_python_version")
    node_ver_sel = context.user_data.get("sel_node_version")
    py_ver   = None if (not py_ver_sel or py_ver_sel == "default") else py_ver_sel
    node_ver = None if (not node_ver_sel or node_ver_sel == "default") else node_ver_sel
    ...
    status_msg = await (update.message or update.callback_query.message).reply_text(
        f"💖 *Setting up {escape_md(name)}* 💖\n\n"
        f"🐍 {escape_md(py_label)}\n"
        f"🟢 {escape_md(node_label)}\n\n"
        f"💞 Starting with love... 💞",
        parse_mode=ParseMode.MARKDOWN,
    )
    ...
    animator = LoveAnimator(
        ...
        header=(
            f"💖 *Setting up {escape_md(name)}* 💖\n"
            f"🐍 {escape_md(py_label)}  •  🟢 {escape_md(node_label)}"
        ),
        interval=1.8,
    )
    animator.start()

    # Step 1: Create venv (with selected python version)
    animator.set_step(f"📁 Creating virtual environment with {py_label}...")
    try:
        proc = await asyncio.wait_for(
            create_subprocess_exec(py_bin, "-m", "venv", os.path.join(pdir, "venv"),
                                   stdout=PIPE, stderr=PIPE),
            timeout=120,
        )
        ...
        animator.add_result("❌ venv timed out")
    except FileNotFoundError:
        animator.add_result(f"❌ {py_label} not found on host")
    except Exception as e:
        animator.add_result(f"❌ venv error: {e}")
    ...
    if os.path.exists(pkg_json_path):
        ...
        try:
            ...
            if proc_n.returncode == 0:
                animator.add_result(f"✅ npm packages installed ({node_label})")
            else:
                animator.add_result(f"❌ npm install failed: {stderr_n.decode()[:300]}")
        except asyncio.TimeoutError:
            animator.add_result("❌ npm install timed out")
        except FileNotFoundError:
            animator.add_result(f"❌ npm not found for {node_label}")
        except Exception as e:
            animator.add_result(f"❌ npm error: {e}")
    ...
    if os.path.exists(req_path) and os.path.exists(pip_path):
        animator.set_step(f"📦 Installing requirements with {py_label}...")
        try:
            proc = await asyncio.wait_for(
                create_subprocess_exec(
            ...
            stdout, stderr = await asyncio.wait_for(proc.communicate(), timeout=600)
            if proc.returncode == 0:
                animator.add_result("✅ Requirements installed")
            else:
                animator.add_result(f"❌ pip install failed: {stderr.decode()[:300]}")
        except asyncio.TimeoutError:
            animator.add_result("❌ pip install timed out")
        except Exception as e:
            animator.add_result(f"❌ pip error: {e}")
        ...
        if os.path.exists(pip_path):
            try:
                ...
                animator.add_result(f"✅ {pkg_count} packages verified")
            except Exception:
                animator.add_result("⚠️ Could not verify packages")
    elif not os.path.exists(pkg_json_path):
        animator.add_result("ℹ️ No requirements.txt found")

    # collect results before stopping animator
    results = list(animator.results)
    ...
    # Save to DB (with chosen runtime versions)
    await projects_col.insert_one({
        "user_id":        uid,
        "name":           name,
        "run_command":    default_cmd,
        "python_version": py_ver,    # may be None => system default
        "node_version":   node_ver,  # may be None => system default
    ...
    result_text += f"\n🐍 Runtime: {escape_md(py_label)}"
    result_text += f"\n🟢 Node:    {escape_md(node_label)}"
...
async def cb_admin_delete_backups(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Count backups in both DBs
    primary_count = await backups_col.count_documents({})
    secondary_count = 0
    if db_2 is not None:
        try:
            secondary_count = await db_2["backups"].count_documents({})
        except Exception as e:
            logger.warning(f"Secondary backup count failed: {e}")

    text = (
        "⚠️ *Delete ALL Backups?*\n\n"
        f"📂 Primary DB backups: `{primary_count}`\n"
        f"📂 Secondary DB backups: `{secondary_count}`\n\n"
        "Ye action *permanent* hai aur *undo nahi* ho sakta.\n"
        "Project files chhediyaa nahi jayengi — sirf MongoDB backups delete honge."
    )
    kb = InlineKeyboardMarkup([
...
async def cb_admin_delete_backups_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Secondary DB — wipe its backups collection if configured
    if db_2 is not None:
        try:
            res2 = await db_2["backups"].delete_many({})
            secondary_deleted = res2.deleted_count
        except Exception as e:
            errors.append(f"Secondary DB error: {e}")
            logger.error(f"Delete all backups (secondary) failed: {e}")

    logger.info(
        f"Admin wiped backups: primary={primary_deleted}, secondary={secondary_deleted}"
    )

    lines = [
        "✅ *All Backups Deleted!*\n",
        f"📂 Primary DB: `{primary_deleted}` documents removed",
    ]
    if db_2 is not None:
        lines.append(f"📂 Secondary DB: `{secondary_deleted}` documents removed")
    else:
        lines.append("ℹ️ Secondary DB not configured — skipped")
...
def build_application() -> Application:
    ...
    new_proj_conv = ConversationHandler(
        ...
        states={
            ...
            NEW_PROJECT_VERSION: [
                CallbackQueryHandler(cb_version_pick,    pattern=r"^ver_(py|node|noop)(:|$)"),
                CallbackQueryHandler(cb_version_confirm, pattern="^ver_confirm$"),
                CallbackQueryHandler(new_project_cancel, pattern="^back_start$"),
            ],
        },
        fallbacks=[
