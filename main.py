...
PORT = int(os.getenv("PORT", "5000"))          # Single port for both keep-alive AND file manager
...
(
    ...
    ADMIN_UNBAN_ID,
    ADMIN_BROADCAST_MSG,
    ADMIN_BROADCAST_SPECIFIC_ID,
    ADMIN_BROADCAST_SPECIFIC_MSG,
    ADMIN_MSG_USER_ID,
    ADMIN_MSG_USER_MSG,
) = range(14)
...
#  In-memory state
# ──────────────────────────────────────────
# running_processes: {f"{user_id}:{project_name}": Popen}
running_processes: dict[str, subprocess.Popen] = {}

# Token store: token -> {"user_id", "project_name", "expiry"}
# Shared between bot callbacks and Flask routes
fm_tokens: dict[str, dict] = {}

# ──────────────────────────────────────────
#  File Manager HTML Template (dark theme + CodeMirror)
# ──────────────────────────────────────────
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .file-item {
    ...
    padding: 7px 14px; cursor: pointer; border-radius: 4px; margin: 1px 6px;
  ...
  .upload-label {
    display: block; padding: 8px; border: 2px dashed var(--border);
  ...
  .toast {
    padding: 12px 18px; border-radius: 8px; font-size: 13px; min-width: 220px;
  ...
  .modal input { width: 100%; padding: 8px 12px; background: var(--bg); border: 1px solid var(--border); border-radius: 6px; color: var(--text); font-size: 14px; margin-bottom: 12px; }
...
<div class="layout">
  <div class="sidebar">
    <div class="sidebar-header">
      📂 Files
      <button class="btn btn-primary" style="padding:4px 10px;font-size:11px" onclick="showNewFileModal()">+ New</button>
    </div>
    <div class="sidebar-actions">
      <button class="btn btn-secondary" onclick="showNewFolderModal()">📁 Folder</button>
      <button class="btn btn-secondary" onclick="refreshFiles()">🔄</button>
    </div>
    <div class="upload-area">
      <label class="upload-label" for="upload-input">⬆️ Upload File</label>
      <input type="file" id="upload-input" multiple onchange="uploadFiles(this)">
    </div>
  ...
  <div class="editor-panel" id="editor-panel">
    ...
    <div class="editor-toolbar" id="editor-toolbar" style="display:none">
      ...
      <button class="btn btn-warning" onclick="downloadCurrentFile()">⬇️ Download</button>
...
<div class="modal-overlay" id="new-file-modal">
  <div class="modal">
    <h3>📄 New File</h3>
    <input type="text" id="new-file-name" placeholder="filename.py" autofocus>
...
<div class="modal-overlay" id="new-folder-modal">
  <div class="modal">
    <h3>📁 New Folder</h3>
    <input type="text" id="new-folder-name" placeholder="folder_name" autofocus>
    <div class="modal-buttons">
...
function getToken() { return "?token=" + TOKEN; }
...
function getIcon(name, isDir) {
  ...
  return {py:"🐍",js:"📜",json:"📋",txt:"📄",md:"📝",html:"🌐",css:"🎨",sh:"⚙️",zip:"📦",log:"📋",env:"🔑",ini:"⚙️",cfg:"⚙️",yaml:"📋",yml:"📋"}[ext] || "📄";
}

function getMode(name) {
  const ext = name.split(".").pop().toLowerCase();
  return {py:"python",js:"javascript",json:"javascript",html:"xml",css:"css",sh:"shell",bash:"shell"}[ext] || "python";
}

async function loadFiles(path="") {
  currentPath = path;
  const res = await fetch(BASE + "/api/list" + getToken() + "&path=" + encodeURIComponent(path));
  ...
  if (path) {
    ...
    li.onclick = () => loadFiles(path.split("/").slice(0,-1).join("/"));
    ul.appendChild(li);
  }
  for (const item of data.items || []) {
    ...
    li.innerHTML = `
      ...
      <div class="file-actions">
        ${!item.is_dir ? '<button onclick="openFile(\''+item.path+'\',event)" title="Edit">✏️</button>' : ''}
        <button onclick="showRenameModal('${item.path}','${item.name}',event)" title="Rename">🏷️</button>
        ${!item.is_dir ? '<button onclick="downloadFile(\''+item.path+'\',event)" title="Download">⬇️</button>' : ''}
        <button onclick="deleteItem('${item.path}',event)" title="Delete">🗑️</button>
      </div>
    `;
    if (item.is_dir) {
      li.querySelector(".file-name").onclick = (e) => { e.stopPropagation(); loadFiles(item.path); };
    } else {
      li.querySelector(".file-name").onclick = (e) => { e.stopPropagation(); openFile(item.path); };
    }
    ul.appendChild(li);
  }
...
async function openFile(path, e) {
  ...
  const res = await fetch(BASE + "/api/read" + getToken() + "&path=" + encodeURIComponent(path));
  const data = await res.json();
  ...
  if (!editor) {
    editor = CodeMirror.fromTextArea(document.getElementById("code-editor"), {
      ...
      lineWrapping: false, scrollbarStyle: "native",
    });
    // Make editor fill the wrap
...
async function saveFile() {
  ...
  const res = await fetch(BASE + "/api/write" + getToken(), {
    method: "POST",
...
function downloadFile(path, e) {
  if (e) e.stopPropagation();
  window.open(BASE + "/api/download" + getToken() + "&path=" + encodeURIComponent(path));
}

async function deleteItem(path, e) {
  if (e) e.stopPropagation();
  if (!confirm("Delete " + path + "?\\nThis cannot be undone.")) return;
  const res = await fetch(BASE + "/api/delete" + getToken(), {
    method: "POST",
    headers: {"Content-Type": "application/json"},
    body: JSON.stringify({ path })
  });
  const data = await res.json();
  if (data.ok) { toast("🗑️ Deleted: " + path, "success"); if (currentFile === path) closeEditor(); refreshFiles(); }
...
function showRenameModal(path, name, e) {
  if (e) e.stopPropagation();
...
async function createNewFolder() {
  ...
  if (!name) return;
  const path = currentPath ? currentPath + "/" + name : name;
  const res = await fetch(BASE + "/api/mkdir" + getToken(), {
    method: "POST",
    headers: {"Content-Type": "application/json"},
    body: JSON.stringify({ path })
  });
...
async function renameItem() {
  ...
  const res = await fetch(BASE + "/api/rename" + getToken(), {
    method: "POST",
    headers: {"Content-Type": "application/json"},
  ...
  if (data.ok) { toast("✏️ Renamed to " + newName, "success"); if (currentFile === oldPath) { currentFile = newPath; document.getElementById("current-file-label").textContent = "📄 " + newPath; } refreshFiles(); }
  else toast("❌ " + data.error, "error");
}
...
async function uploadFiles(input) {
  ...
  for (const file of files) {
    ...
    const res = await fetch(BASE + "/api/upload" + getToken(), { method: "POST", body: formData });
    const data = await res.json();
...
# ──────────────────────────────────────────
#  Single Flask App
#  Serves: keep-alive "/" AND all file manager routes "/files/..."
# ──────────────────────────────────────────
...
def file_manager_ui(user_id: int, project_name: str):
    ...
    base_url = request.host_url.rstrip("/") + f"/files/{user_id}/{project_name}"
...
def api_list(user_id: int, project_name: str):
    ...
    base = PROJECTS_DIR / str(user_id) / project_name
...
def api_rename(user_id: int, project_name: str):
    ...
    base = PROJECTS_DIR / str(user_id) / project_name
    old_target = _safe_path(base, old_rel)
    new_target = _safe_path(base, new_rel)
...
def fmt_uptime(start_ts: float) -> str:
    """Return human-readable uptime string from a unix timestamp."""
...
async def check_premium_expiry(user_id: int) -> dict:
    """Downgrade to free if temp premium has expired. Returns updated doc."""
...
async def sync_project_status(project: dict) -> dict:
    """Check if stored PID is still alive; update DB status if needed."""
    if project.get("status") == "running":
        pid = project.get("pid")
        key = f"{project['user_id']}:{project['project_name']}"
        if not pid or (not process_alive(pid) and key not in running_processes):
...
async def kill_project_process(user_id: int, project_name: str, pid: int | None = None):
...
async def banned_guard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Return True (and reply) if the user is banned."""
...
async def new_project_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if count >= limit:
        plan = "Premium" if limit == 10 else "Free"
        await query.edit_message_text(
            f"❌ You have reached your project limit ({limit} for {plan} plan).\n"
        ...
        return ConversationHandler.END
    await query.edit_message_text("📝 *Enter a name for your new project:*\n\n_(Use letters, numbers, underscores only)_", parse_mode=ParseMode.MARKDOWN)
    return NP_NAME
...
async def new_project_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not safe_name:
        await update.message.reply_text("❌ Invalid name. Use letters, numbers, underscores.")
        return NP_NAME
    user_id = update.effective_user.id
    # Check duplicate
    existing = await projects_col.find_one({"user_id": user_id, "project_name": safe_name})
    if existing:
        await update.message.reply_text(f"❌ You already have a project named *{safe_name}*. Choose another name.", parse_mode=ParseMode.MARKDOWN)
        return NP_NAME
...
async def new_project_done(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_id = update.effective_user.id
    project_name = context.user_data.get("new_project_name")
    files_list = context.user_data.get("new_project_files", [])
    if not project_name:
    ...
    if req_file.exists():
        await context.bot.send_message(
            ...
            text="📦 Found `requirements.txt` — installing packages…",
            parse_mode=ParseMode.MARKDOWN,
        )
        try:
            proc = subprocess.run(
            ...
            # Parse installed/failed packages
            lines = [l for l in output.splitlines() if l.strip()]
            success_lines = [l for l in lines if "Successfully installed" in l or "already satisfied" in l.lower()]
            fail_lines = [l for l in lines if "error" in l.lower() or "failed" in l.lower()]
            install_result = "✅ *Package Installation Results:*\n"
            if success_lines:
                install_result += "\n".join(f"  ✅ {l}" for l in success_lines[:10]) + "\n"
            if fail_lines:
                install_result += "\n".join(f"  ❌ {l}" for l in fail_lines[:5]) + "\n"
            if not success_lines and not fail_lines:
                install_result += "  ℹ️ " + "\n  ℹ️ ".join(lines[:5])
        except subprocess.TimeoutExpired:
            install_result = "⚠️ pip install timed out after 120s"
        except Exception as e:
            install_result = f"❌ pip error: {e}"
    # Determine default run command
    main_py = project_dir / "main.py"
    run_cmd = f"python3 main.py" if main_py.exists() else f"python3 {disk_files[0]}" if disk_files else "python3 main.py"
    # Save to DB
    ...
    msg = (
        ...
        + (f"\n{install_result}" if install_result else "") +
...
# ──────────────────────────────────────────
#  Project Actions (Run / Restart / Logs / Refresh / Delete)
# ──────────────────────────────────────────
...
async def refresh_project(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if proc:
        ...
        if ret is not None:
            ...
            await projects_col.update_one(
                {"user_id": user_id, "project_name": project_name},
                {"$set": {"status": "stopped" if ret == 0 else "error", "exit_code": ret, "uptime_start": None}},
            )
            project["status"] = "stopped" if ret == 0 else "error"
            project["exit_code"] = ret
    await _send_project_dashboard(query, project)
...
async def edit_cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await query.edit_message_text(
        ...
        f"Current: `{current}`\n\n"
        "Send the new run command (e.g. `python3 bot.py`):",
        parse_mode=ParseMode.MARKDOWN,
    )
    return EDIT_CMD
...
async def bot_status_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    running_projects = await projects_col.count_documents({"status": "running"})
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "all_users":
        ...
        cursor = users_col.find({}).skip(page * per_page).limit(per_page)
        ...
        text = f"👥 *All Users* (Page {page+1}/{max(1,(total+per_page-1)//per_page)})\n\n" + "\n".join(lines)
        nav_buttons = []
        if page > 0:
            nav_buttons.append(InlineKeyboardButton("⬅️ Prev", callback_data=f"admin:all_users:{page-1}"))
        if (page + 1) * per_page < total:
            nav_buttons.append(InlineKeyboardButton("Next ➡️", callback_data=f"admin:all_users:{page+1}"))
    ...
    # ── Running Scripts ──
        ...
        cursor = projects_col.find({"status": "running"})
        ...
        await projects_col.update_one(
            ...
            {"$set": {"status": "stopped", "pid": None, "owner_stopped": True}},
        )
        await query.edit_message_text(
    ...
    # Trigger admin input flows
    elif action in ("give_premium", "remove_premium", "temp_premium", "ban", "unban", "broadcast_all", "broadcast_specific", "msg_user"):
        ...
        prompts = {
            ...
            "broadcast_specific": "📨 *Message Specific User*\n\nSend the target user_id:",
        ...
        context.user_data["admin_prompt_chat"] = update.effective_chat.id
...
async def admin_input_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Handle sequential admin inputs (user_id, duration, message, etc.)."""
    if update.effective_user.id != OWNER_ID:
        return ConversationHandler.END
    action = context.user_data.get("admin_action")
    step = context.user_data.get("admin_step", 0)
    ...
    if action == "give_premium":
        ...
        await update.message.reply_text(f"✅ Premium given to user `{uid}`.", parse_mode=ParseMode.MARKDOWN, reply_markup=admin_panel_keyboard())
        context.user_data.clear()
        return ConversationHandler.END
    ...
    if action == "remove_premium":
        ...
        # Enforce limits — delete extra projects if > 1
        extra_cursor = projects_col.find({"user_id": uid}).skip(1)
        extra = await extra_cursor.to_list(length=100)
        for p in extra:
    ...
    if action == "temp_premium":
        if step == 0:
            ...
            context.user_data["admin_step"] = 1
            await update.message.reply_text("⏰ Send duration: e.g. `24h` or `7d` (hours or days):", parse_mode=ParseMode.MARKDOWN)
            return ADMIN_TEMP_PREMIUM_DUR
        else:
            uid = context.user_data["temp_premium_uid"]
            try:
            ...
            await users_col.update_one({"user_id": uid}, {"$set": {"plan": "premium", "premium_expiry": expiry}}, upsert=True)
            await update.message.reply_text(
    ...
    if action == "ban":
        ...
        cursor = projects_col.find({"user_id": uid, "status": "running"})
        running = await cursor.to_list(length=100)
        for p in running:
            await kill_project_process(uid, p["project_name"], p.get("pid"))
            await projects_col.update_one({"_id": p["_id"]}, {"$set": {"status": "stopped"}})
        await update.message.reply_text(f"🚫 User `{uid}` banned and all running projects stopped.", parse_mode=ParseMode.MARKDOWN, reply_markup=admin_panel_keyboard())
        context.user_data.clear()
        return ConversationHandler.END
    ...
    if action == "unban":
        ...
        await update.message.reply_text(f"✅ User `{uid}` unbanned.", parse_mode=ParseMode.MARKDOWN, reply_markup=admin_panel_keyboard())
        context.user_data.clear()
        return ConversationHandler.END
    ...
    if action == "broadcast_all":
        ...
        for u in all_users:
            try:
                await context.bot.send_message(chat_id=u["user_id"], text=f"📢 *Broadcast:*\n\n{text_in}", parse_mode=ParseMode.MARKDOWN)
                success += 1
            except Exception:
                fail += 1
        await update.message.reply_text(f"📢 Broadcast done.\n✅ Sent: {success}\n❌ Failed: {fail}", reply_markup=admin_panel_keyboard())
        context.user_data.clear()
        return ConversationHandler.END
    ...
    if action == "broadcast_specific":
        if step == 0:
            try:
                ...
                await update.message.reply_text("❌ Invalid user_id.")
                return ADMIN_BROADCAST_SPECIFIC_ID
            context.user_data["broadcast_uid"] = uid
            context.user_data["admin_step"] = 1
            await update.message.reply_text("📨 Now send the message:")
            return ADMIN_BROADCAST_SPECIFIC_MSG
        else:
            uid = context.user_data["broadcast_uid"]
            try:
                await context.bot.send_message(chat_id=uid, text=f"📨 *Message from admin:*\n\n{text_in}", parse_mode=ParseMode.MARKDOWN)
                await update.message.reply_text(f"✅ Message sent to `{uid}`.", parse_mode=ParseMode.MARKDOWN, reply_markup=admin_panel_keyboard())
            except Exception as e:
                await update.message.reply_text(f"❌ Failed: {e}", reply_markup=admin_panel_keyboard())
            context.user_data.clear()
            return ConversationHandler.END
    ...
    if action == "msg_user":
        if step == 0:
            ...
            context.user_data["admin_step"] = 1
            await update.message.reply_text("📨 Now send the message:")
            return ADMIN_MSG_USER_MSG
        else:
            uid = context.user_data["msg_uid"]
            try:
                await context.bot.send_message(chat_id=uid, text=f"📨 *Message from admin:*\n\n{text_in}", parse_mode=ParseMode.MARKDOWN)
                await update.message.reply_text(f"✅ Message sent to `{uid}`.", parse_mode=ParseMode.MARKDOWN, reply_markup=admin_panel_keyboard())
            except Exception as e:
...
# ──────────────────────────────────────────
#  Admin plain-text input router
# ──────────────────────────────────────────

async def admin_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Route plain text messages when an admin action is pending."""
    if update.effective_user.id != OWNER_ID:
        return
    action = context.user_data.get("admin_action")
    if not action:
        return
    step = context.user_data.get("admin_step", 0)
    # Map action to expected state
    state_map = {
        "give_premium": ADMIN_GIVE_PREMIUM_ID,
        "remove_premium": ADMIN_REMOVE_PREMIUM_ID,
        "temp_premium": ADMIN_TEMP_PREMIUM_ID if step == 0 else ADMIN_TEMP_PREMIUM_DUR,
        "ban": ADMIN_BAN_ID,
        "unban": ADMIN_UNBAN_ID,
        "broadcast_all": ADMIN_BROADCAST_MSG,
        "broadcast_specific": ADMIN_BROADCAST_SPECIFIC_ID if step == 0 else ADMIN_BROADCAST_SPECIFIC_MSG,
        "msg_user": ADMIN_MSG_USER_ID if step == 0 else ADMIN_MSG_USER_MSG,
    }
    fake_state = state_map.get(action, -1)
    if fake_state >= 0:
...
async def on_startup(application: Application):
    """Mark all previously 'running' projects as stopped (bot restarted)."""
    await projects_col.update_many(
        {"status": "running"},
        {"$set": {"status": "stopped", "pid": None, "uptime_start": None}},
    )
    logger.info("Cleaned up stale running projects on startup.")
...
def run_flask():
    """Run the single Flask app (keep-alive + file manager) in a background thread."""
    logger.info(f"Starting Flask server on port {PORT} (keep-alive + file manager)…")
...
def main():
    ...
    # Start the single Flask app (keep-alive "/" + all file manager routes) in a daemon thread
    ...
    # Admin plain-text router (lower priority, only fires when admin_action set)
    tg_app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, admin_text_router))
