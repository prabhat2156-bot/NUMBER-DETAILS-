...
FM_BASE_URL  = os.environ.get("FM_BASE_URL", f"http://localhost:{PORT}")

# Plan limits: {plan_name: max_projects}
PLAN_LIMITS = {
    "free":    1,
    "basic":   3,
    "premium": 10,
...
async def is_user_banned(user_id: int) -> bool:
    doc = await users_col.find_one({"_id": user_id}, {"banned": 1})
...
async def check_premium_expiry(user_id: int) -> None:
    ...
    doc = await users_col.find_one({"_id": user_id}, {"plan": 1, "premium_expiry": 1})
    if not doc:
        return
    if doc.get("plan") in ("basic", "premium") and doc.get("premium_expiry"):
...
def _build_run_command(cmd: str, project_path: Path) -> list[str]:
    """Build the subprocess command list, resolving virtualenv if present."""
...
def start_project_process(user_id: int, project_id: str, project_name: str, cmd: str) -> Optional[subprocess.Popen]:
    ...
    try:
        command = _build_run_command(cmd, project_path)
        log_file = open(str(log_path), "a", encoding="utf-8")
        log_file.write(f"\n\n{'='*40}\n[{datetime.utcnow().isoformat()}] Starting: {' '.join(command)}\n{'='*40}\n")
        log_file.flush()
        ...
        logger.error("Failed to start project %s: %s", project_name, exc)
        return None
...
def admin_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💎 Give Premium",  callback_data="admin_give_prem_start"),
         InlineKeyboardButton("🚫 Remove Prem",   callback_data="admin_rem_prem_start")],
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"🤖 *God Madara Hosting Bot*\n"
        f"Host your Python bots effortlessly.\n\n"
        f"🔹 Upload your bot files\n"
        f"🔹 Set a run command\n"
        f"🔹 Start / stop / restart anytime\n"
        f"🔹 View live logs\n"
        f"🔹 Browse & edit files in browser\n\n"
...
async def main_menu_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        f"🏠 *Main Menu*\n\n"
        f"Welcome back, *{user.full_name}*! Choose an option:"
    )
...
async def bot_status_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"  OS: `{stats['os']}` | Python `{stats['python']}`\n\n"
        f"📈 *Stats*\n"
        f"  👥 Total Users: `{total_users}`\n"
        f"  📁 Total Projects: `{total_projects}`\n"
        f"  ▶️ Running Now: `{running_count}`\n\n"
        f"💾 *Backup Stats*\n"
        f"  Files: `{backup_stats['total_files']}`\n"
        f"  Size: `{backup_stats['total_size']}`\n"
        f"  Projects: `{backup_stats['total_projects']}`"
    )
...
async def premium_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer()
    user_doc = await users_col.find_one({"_id": update.effective_user.id})
    ...
    text = (
        f"💎 *Premium Plans*\n\n"
        f"Your current plan: {get_plan_label(plan)}\n"
        f"Expiry: `{expiry_str}`\n\n"
        f"🆓 *Free* — 1 project\n"
        f"⭐ *Basic* — 3 projects\n"
        f"💎 *Premium* — 10 projects\n\n"
        f"To upgrade, contact the owner."
    )
    kb = back_to_main()
    if plan == "free":
...
async def project_dashboard_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    entry       = running_processes.get(project_id, {})
    uptime      = uptime_str(entry["started_at"]) if is_running and "started_at" in entry else "—"
    pid_display = entry.get("pid", "—") if is_running else "—"
    status_icon = "🟢 Running" if is_running else "🔴 Stopped"
    project_path = get_project_path(proj["user_id"], proj["name"])
    disk_usage   = bytes_human(sum(f.stat().st_size for f in project_path.rglob("*") if f.is_file())) if project_path.exists() else "—"
    text = (
        f"📁 *{proj['name']}*\n\n"
        f"Status: {status_icon}\n"
        f"PID: `{pid_display}`\n"
        f"Uptime: `{uptime}`\n"
        f"Run Command: `{proj.get('run_cmd', 'N/A')}`\n"
        f"Disk: `{disk_usage}`\n"
        f"Admin Stopped: `{proj.get('admin_stopped', False)}`\n"
        f"Created: `{proj.get('created_at', 'N/A')}`"
    )
    await query.edit_message_text(
        text,
        parse_mode=ParseMode.MARKDOWN,
...
async def run_project_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if proj.get("admin_stopped"):
        await query.answer("🚫 This project was stopped by an admin and cannot be restarted.", show_alert=True)
        return
    if _is_process_alive(project_id):
        await query.answer("⚠️ Already running!", show_alert=True)
        return
    proc = start_project_process(proj["user_id"], project_id, proj["name"], proj.get("run_cmd", "python main.py"))
    if proc:
        await projects_col.update_one({"_id": ObjectId(project_id)}, {"$set": {"status": "running", "admin_stopped": False}})
        await query.edit_message_text(
...
async def restart_project_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not proj:
        ...
        return
    if proj.get("admin_stopped"):
        await query.answer("🚫 This project was stopped by an admin.", show_alert=True)
        return
...
async def fm_project_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        f"🔗 [Open File Manager]({fm_url})\n\n"
        f"⚠️ Link expires in 60 minutes.\n"
...
async def np_got_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        ...
        f"You can send:\n• A ZIP file (recommended)\n• A single `.py` file\n• Multiple files (send them one by one, then type /done)",
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def np_got_file(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not doc:
        ...
        await update.message.reply_text("Please send a file or type /done when finished.")
        return NP_FILE
    file_obj = await doc.get_file()
    dest = project_path / doc.file_name
    await file_obj.download_to_drive(str(dest))
    context.user_data["np_files"].append(doc.file_name)
    await update.message.reply_text(f"📥 Received: `{doc.file_name}`\n\nSend more files or type /done.", parse_mode=ParseMode.MARKDOWN)
    # If it's a ZIP, extract immediately and confirm
    if doc.file_name.lower().endswith(".zip"):
        await update.message.reply_text("🗜 ZIP detected. Type /done to finalize and extract.", parse_mode=ParseMode.MARKDOWN)
    return NP_FILE
...
async def np_done_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not files:
        await update.message.reply_text("❌ No files received. Please upload at least one file.")
        return NP_FILE
    # Extract ZIP files
    for fname in files:
        ...
        if fname.lower().endswith(".zip") and fpath.exists():
            try:
                ...
                await update.message.reply_text(f"⚠️ ZIP extraction failed: {exc}")
    ...
    if req_file.exists():
        ...
        try:
            ...
            result = subprocess.run(
                [pip_exec, "install", "-r", str(req_file), "--quiet"],
                cwd=str(project_path),
                capture_output=True,
                text=True,
                timeout=120,
            ...
            await update.message.reply_text(f"⚠️ pip install error: {exc}")
...
async def np_got_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not project_id:
        ...
        return ConversationHandler.END
    await projects_col.update_one({"_id": ObjectId(project_id)}, {"$set": {"run_cmd": cmd}})
    await update.message.reply_text(
...
async def admin_panel_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if update.effective_user.id != OWNER_ID:
        ...
        return
    await query.edit_message_text("👑 *Admin Panel*", parse_mode=ParseMode.MARKDOWN, reply_markup=admin_keyboard())
...
async def admin_users_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for u in users:
        ...
        name   = u.get("full_name") or u.get("username") or str(u["_id"])
        lines.append(f"• [{name}](tg://user?id={u['_id']}) — {plan}{banned}")
        buttons.append([InlineKeyboardButton(f"{name}", callback_data=f"admin_user_detail:{u['_id']}")])
    nav = []
    if page > 0:
...
async def admin_running_cb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not running_processes:
        await query.edit_message_text("⚙️ No projects currently running.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Admin", callback_data="admin_panel")]]))
        return
    lines = ["⚙️ *Running Projects*\n"]
    buttons = []
    for pid, entry in running_processes.items():
        proj = await projects_col.find_one({"_id": ObjectId(pid)})
        if not proj:
            continue
        user = await users_col.find_one({"_id": proj["user_id"]})
        uname    = user.get("username") or str(proj["user_id"]) if user else str(proj["user_id"])
        uptime   = uptime_str(entry["started_at"])
        pid_num  = entry.get("pid", "?")
        lines.append(f"• *{proj['name']}* (@{uname}) — PID `{pid_num}` — Up {uptime}")
        buttons.append([InlineKeyboardButton(f"⏹ Stop {proj['name']}", callback_data=f"admin_stop_proj:{pid}")])
...
async def admin_give_prem_got(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        return GIVE_PREM_ID
    await users_col.update_one({"_id": uid}, {"$set": {"plan": "premium", "premium_expiry": None}}, upsert=True)
...
async def admin_rem_prem_got(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("❌ Invalid ID.")
        return REM_PREM_ID
    await users_col.update_one({"_id": uid}, {"$set": {"plan": "free", "premium_expiry": None}})
...
async def admin_ban_got(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("❌ Invalid ID.")
        return BAN_ID
    await users_col.update_one({"_id": uid}, {"$set": {"banned": True}}, upsert=True)
...
async def admin_unban_got(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        await update.message.reply_text("❌ Invalid ID.")
        return UNBAN_ID
    await users_col.update_one({"_id": uid}, {"$set": {"banned": False}})
...
async def admin_bc_all_got(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    sent     = 0
    failed   = 0
    async for user in users_col.find({"banned": {"$ne": True}}, {"_id": 1}):
        try:
            await bot.send_message(user["_id"], f"📢 *Broadcast*\n\n{msg}", parse_mode=ParseMode.MARKDOWN)
            sent += 1
        except Exception:
            failed += 1
    await update.message.reply_text(
...
# ═══════════════════════════════════════════════════════════════════════════
# FILE MANAGER — Flask app (runs in daemon thread)
# ═══════════════════════════════════════════════════════════════════════════
...
<div class="breadcrumb">
  <a href="{{ url_for('fm_browse', token=token, rel='') }}">🏠 Root</a>
  {% for crumb in breadcrumbs %}
    <span>/</span>
    <a href="{{ url_for('fm_browse', token=token, rel=crumb.rel) }}">{{ crumb.name }}</a>
  {% endfor %}
</div>
<div class="toolbar">
...
<table class="file-table">
  ...
  <tr>
    <td>
      ...
      {% if f.is_dir %}
        <a class="file-link" href="{{ url_for('fm_browse', token=token, rel=f.rel) }}">{{ f.name }}</a>
      {% elif f.editable %}
        <a class="file-link" href="{{ url_for('fm_edit_file', token=token, rel=f.rel) }}">{{ f.name }}</a>
      {% else %}
        <a class="file-link" href="{{ url_for('fm_download_file', token=token, rel=f.rel) }}" download>{{ f.name }}</a>
      {% endif %}
    </td>
    <td>{{ f.size }}</td>
    <td>{{ f.mtime }}</td>
    <td>
      <div class="actions">
        {% if not f.is_dir and f.editable %}
          <a class="btn btn-muted" href="{{ url_for('fm_edit_file', token=token, rel=f.rel) }}">✏️</a>
        {% endif %}
        {% if not f.is_dir %}
          <a class="btn btn-muted" href="{{ url_for('fm_download_file', token=token, rel=f.rel) }}" download>⬇️</a>
        {% endif %}
...
<div class="modal-overlay" id="modal-newfile">
  <div class="modal">
    ...
    <form method="POST" action="{{ url_for('fm_create_file', token=token) }}">
...
<div class="modal-overlay" id="modal-newfolder">
  <div class="modal">
    ...
    <form method="POST" action="{{ url_for('fm_create_folder', token=token) }}">
      <input type="hidden" name="dir" value="{{ current_rel }}">
      <div class="form-group">
...
<div class="modal-overlay" id="modal-rename">
  <div class="modal">
    <h3>🔤 Rename</h3>
    <form method="POST" action="{{ url_for('fm_rename_item', token=token) }}">
...
<div class="modal-overlay" id="modal-delete">
  <div class="modal">
    ...
    <form method="POST" action="{{ url_for('fm_delete_item', token=token) }}">
...
function uploadFiles(input) {
  ...
  fetch('{{ url_for("fm_upload_files", token=token) }}', { method: 'POST', body: fd })
    .then(r => r.json())
...
<div class="editor-area">
  <div class="editor-header">
    ...
    <div style="display:flex;gap:8px">
      <a class="btn btn-muted" href="{{ url_for('fm_browse', token=token, rel=parent_rel) }}">⬅️ Back</a>
...
def get_token_and_base(token: str):
    ...
    if not data:
        abort(403)
    # Resolve base path via meta file written by bot
...
@flask_app.route("/")
def fm_index():
    return jsonify({"status": "ok", "service": "God Madara Hosting Bot", "version": "1.0"})
...
def fm_edit_file(token):
    rel = request.args.get("rel", "")
    _, base = get_token_and_base(token)
    target  = safe_path(base, rel)
    if not target or not target.is_file():
        flash("File not found.", "error")
        return redirect(url_for("fm_browse", token=token, rel=""))
    try:
...
def fm_create_file(token):
    ...
    return redirect(url_for("fm_edit_file", token=token, rel=new_rel))


@flask_app.route("/fm/<token>/create_folder", methods=["POST"])
def fm_create_folder(token):
    _, base    = get_token_and_base(token)
    dir_rel    = request.form.get("dir", "")
...
def fm_rename_item(token):
    ...
    if new_target.exists():
        ...
        return redirect(url_for("fm_browse", token=token, rel=parent_rel))
...
def run_flask() -> None:
    """Run the Flask file manager server in a daemon thread."""
    TOKENS_DIR.mkdir(exist_ok=True)
...
if __name__ == "__main__":
    ...
    # Start Flask file manager in a background daemon thread
    ...
    # Build and run the Telegram bot in the main thread
