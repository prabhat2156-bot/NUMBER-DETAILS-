...
async def _np_finalize(update: Update, context: ContextTypes.DEFAULT_TYPE, reply_func) -> int:
    """Shared finalization logic for Done Uploading (button or /done command)."""
    user_id = update.effective_user.id
    name    = context.user_data.get("np_name", "")
    if not name:
    ...
    if req_file.exists():
        await reply_func("📦 Installing requirements...")
        try:
            ...
            if result.returncode == 0:
                await reply_func("✅ Requirements installed.")
            else:
                await reply_func(f"⚠️ pip install had issues:\n```{result.stderr[:500]}```", parse_mode=ParseMode.MARKDOWN)
        except Exception as exc:
