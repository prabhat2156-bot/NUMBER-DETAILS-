...
async def cb_bot_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Measure actual DB ping (single lightweight command)
    t0 = time.time()
    await db.command("ping")
    db_ping = int((time.time() - t0) * 1000)

    # Measure bot API ping
    t1 = time.time()
...
def project_dashboard_kb(user_id: int, project_name: str, auto_restart: bool = True) -> InlineKeyboardMarkup:
    pn = project_name
...
async def cb_project_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, project_dashboard_text(p), reply_markup=project_dashboard_kb(uid, name, p.get("auto_restart", True)), parse_mode=ParseMode.MARKDOWN)
...
async def _finalize_new_project(update: Update, context: ContextTypes.DEFAULT_TYPE, via_message: bool):
    ...
    # Save to DB (with auto_restart fields)
...
async def cb_admin_running(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for p in running:
            ...
            if os.path.exists(pdir):
                script_files = [f for f in os.listdir(pdir)
                               if os.path.isfile(os.path.join(pdir, f))
                               and f not in ("output.log",)
                               and not f.endswith(".pyc")]
                files_str = ", ".join(script_files[:5]) if script_files else "N/A"
            else:
                files_str = "N/A"
...
async def env_add_key(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not re.match(r"^[A-Za-z_][A-Za-z0-9_]*$", key):
        await update.message.reply_text("❌ Invalid variable name. Use letters, numbers, underscores. Try again:")
        return ENV_ADD_KEY
            ...
            for p in running:
                ...
                if pid and not psutil.pid_exists(pid):
                        ...
                        if restart_count < 3:
                            ...
                            if notification_bot:
                                try:
                                    await notification_bot.send_message(
                                        chat_id=p["user_id"],
                                        text=f"⚠️ *Auto-Restart Limit Reached*\n\n"
                    ...
                    # Feature 2: Crash notification (auto_restart is OFF)
                        ...
                        if notification_bot:
                            try:
                                # Read last 10 lines of log for the notification
                                log_path = os.path.join(project_dir(p["user_id"], p["name"]), "output.log")
                                error_lines = ""
                                if os.path.exists(log_path):
                                    with open(log_path, "r", errors="replace") as f:
                                        lines = f.readlines()
                                    error_lines = "".join(lines[-10:]).strip()
                                ...
                                await notification_bot.send_message(
                                    ...
                                    text=f"❌ *Project Crashed*\n\n"
                                         ...
                                         f"Auto-Restart: OFF\n\n"
                                         f"📋 *Last Log Lines:*\n```\n{error_lines}\n```\n\n"
                                         f"_Use /start to check your project._",
                                    parse_mode=ParseMode.MARKDOWN,
                                )
                            except Exception:
                                pass
