...
BASE_URL = os.environ.get("BASE_URL", os.environ.get("RENDER_EXTERNAL_URL", f"http://localhost:{PORT}"))
...
async def new_project_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Create virtualenv (ASYNC - non-blocking)
    status_lines.append("\n🐍 *Creating virtualenv...*")
    ...
    try:
        ...
        if proc.returncode == 0:
            ...
            status_lines.append(f"  ❌ Virtualenv error: {stderr.decode()[:200]}")
    except asyncio.TimeoutError:
        status_lines.append("  ❌ Virtualenv creation timed out")
    except Exception as e:
        status_lines.append(f"  ❌ Virtualenv exception: {e}")

    # Install requirements.txt if present (ASYNC - non-blocking)
    ...
    if os.path.exists(req_file):
        ...
        try:
            progress_text = "\n".join(status_lines) + "\n\n⏳ Installing packages..."
            await msg.edit_text(f"⚙️ *Setting up `{project_name}`*\n\n{progress_text}", parse_mode=ParseMode.MARKDOWN)
        except Exception:
            pass

        try:
            # Install ALL requirements at once (faster + non-blocking)
            proc = await create_subprocess_exec(
            ...
            if proc.returncode == 0:
                status_lines.append("  ✅ All packages installed successfully")
            else:
                err_text = stderr.decode().strip()[-300:]
                status_lines.append(f"  ⚠️ Some packages may have failed:\n```\n{err_text}\n```")
        except asyncio.TimeoutError:
            status_lines.append("  ⏱ Installation timed out (5 min limit)")
        except Exception as e:
            status_lines.append(f"  ❌ Install error: {e}")

        # Verify installation
        status_lines.append("\n🔍 *Verifying installation...*")
        try:
            with open(req_file, "r") as rf:
                packages = [l.strip().split("==")[0].split(">=")[0].split("<=")[0].split("~=")[0].strip() 
                           for l in rf if l.strip() and not l.startswith("#") and not l.startswith("-")]
            
            verify_proc = await create_subprocess_exec(
                pip_path, "list", "--format=columns",
                stdout=PIPE, stderr=PIPE
            )
            v_stdout, _ = await asyncio.wait_for(verify_proc.communicate(), timeout=30)
            installed = v_stdout.decode().lower()
            
            verified = 0
            failed_pkgs = []
            for pkg in packages:
                if pkg and pkg.lower().replace("-", "_").replace(".", "_") in installed.replace("-", "_").replace(".", "_"):
                    verified += 1
                elif pkg:
                    failed_pkgs.append(pkg)
            
            status_lines.append(f"  ✅ Verified: {verified}/{len(packages)} packages")
            if failed_pkgs:
                status_lines.append(f"  ❌ Not found: {', '.join(failed_pkgs[:5])}")
        except Exception:
            status_lines.append("  ⚠️ Verification skipped")
    else:
    ...
    final_text = (
        ...
        f"_(Change this in project dashboard)_"
    )
    kb = InlineKeyboardMarkup([
