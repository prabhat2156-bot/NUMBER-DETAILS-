...
async def cb_project_dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await safe_edit(query, project_dashboard_text(p), reply_markup=project_dashboard_kb(uid, name), parse_mode=ParseMode.MARKDOWN)
...
async def cb_run(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await safe_edit(query, f"❌ Failed to start: {escape_md(str(e))}", parse_mode=ParseMode.MARKDOWN)
