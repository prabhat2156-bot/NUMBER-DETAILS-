...
# Timeouts: agar DB unreachable hai to 5 sec mein fail ho — hang nahi karega
_MONGO_OPTS = dict(
    serverSelectionTimeoutMS=5000,
    connectTimeoutMS=5000,
    socketTimeoutMS=30000,
)
mongo_client = AsyncIOMotorClient(MONGODB_URI, **_MONGO_OPTS)
...
def _load_extra_databases():
    ...
    if legacy_uri and legacy_name and legacy_name not in seen_names:
        try:
            client = AsyncIOMotorClient(legacy_uri, **_MONGO_OPTS)
    ...
    for i in range(1, 51):  # supports up to 50 extra DBs
        ...
        if not uri or not name or name in seen_names:
            continue
        try:
            client = AsyncIOMotorClient(uri, **_MONGO_OPTS)
            extra_clients.append(client)
...
async def healthy_backup_targets() -> list:
    """
    Sirf un DBs ko return karta hai jo abhi reachable hain.
    Returns: list of (db_name, backup_collection) tuples — sharding ke liye.
    Dead DBs ko skip karta hai taaki backup hang na ho.
    """
    targets = []
    # Primary check
    try:
        await db.command("ping")
        targets.append((DATABASE_NAME, backups_col))
    except Exception as e:
        logger.warning(f"Primary DB unreachable for backup: {e}")
    # Extra DBs check
    for entry in extra_dbs:
        try:
            await entry["db"].command("ping")
            targets.append((entry["name"], entry["db"]["backups"]))
        except Exception as e:
            logger.warning(f"Extra DB '{entry['name']}' unreachable for backup: {e}")
    return targets

def pick_backup_col_from(targets: list, user_id: int, project_name: str):
    """
    Sharding from a pre-filtered (healthy) targets list.
    Returns: (db_name, backup_collection)
    """
    if not targets:
        return (DATABASE_NAME, backups_col)  # fallback
    if len(targets) == 1:
        return targets[0]
    import hashlib
...
async def cb_admin_backup_now(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        # STEP 1: Pehle healthy DBs ka pata laga (dead DBs skip ho jayenge)
        healthy_targets = await healthy_backup_targets()
        if not healthy_targets:
            raise RuntimeError("Koi DB reachable nahi — backup nahi ho sakta")

        healthy_names = [t[0] for t in healthy_targets]
        skipped_count = (1 + len(extra_dbs)) - len(healthy_targets)

        # User ko progress dikhao
        await safe_edit(
            query,
            f"💾 *Backup in progress...*\n\n"
            f"🗄 Healthy DBs: `{len(healthy_targets)}`"
            + (f" ({skipped_count} dead skipped)" if skipped_count > 0 else "")
            + f"\n📋 Reading projects...",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]),
            parse_mode=ParseMode.MARKDOWN,
        )
        ...
        for idx, proj in enumerate(all_projects):
            ...
            if files_data:
                # SHARDING: sirf healthy DBs mein se assign karo
                target_db_name, target_col = pick_backup_col_from(healthy_targets, uid, name)
                doc = {
                    "type":         "file_backup",
                    "user_id":      uid,
                    "project_name": name,
                ...
                try:
                    # Upsert on target — fast (1 op instead of N deletes + 1 insert)
                    await target_col.replace_one(
                        {"type": "file_backup", "user_id": uid, "project_name": name},
                        doc,
                        upsert=True,
                    )
                    # Old duplicates from OTHER healthy DBs cleanup karo (one-time migration)
                    for other_name, other_col in healthy_targets:
                        if other_name == target_db_name:
                            continue
                        try:
                            await other_col.delete_many({
                                "type": "file_backup",
                                "user_id": uid,
                                "project_name": name,
                            })
            ...
            # Progress update — har 5 projects pe (avoid telegram rate limit)
            if (idx + 1) - last_progress_update >= 5 and len(all_projects) > 5:
                last_progress_update = idx + 1
                try:
                    await safe_edit(
                        query,
                        f"💾 *Backup in progress...*\n\n"
                        f"📋 Processed: `{idx + 1}/{len(all_projects)}`\n"
                        f"📄 Files: `{total_files}`\n"
                        f"📦 Size: `{fmt_bytes(total_size)}`",
                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back", callback_data="admin_panel")]]),
                        parse_mode=ParseMode.MARKDOWN,
                    )
                except Exception:
...
async def backup_task():
    ...
    while True:
        ...
        try:
            # Healthy DBs check — dead ones skip ho jayenge
            healthy_targets = await healthy_backup_targets()
            if not healthy_targets:
                logger.warning("Auto-backup skipped: koi DB reachable nahi")
                continue
            ...
            for proj in all_projects:
                ...
                if files_data:
                    # SHARDING: sirf healthy DBs se assign
                    target_db_name, target_col = pick_backup_col_from(healthy_targets, uid, name)
                    try:
                        await target_col.replace_one(
                            {"type": "file_backup", "user_id": uid, "project_name": name},
                        ...
                        # Cleanup duplicates from OTHER healthy DBs
                        for other_name, other_col in healthy_targets:
                            if other_name == target_db_name:
                                continue
                            try:
                        ...
                        logger.warning(f"Auto-backup write failed for {name} on {target_db_name}: {e}")
